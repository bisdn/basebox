...
/*static*/ std::string c8021core::script_path_port_up =
    std::string("/var/lib/basebox/port-up.sh");
/*static*/ std::string c8021core::script_path_port_down =
    std::string("/var/lib/basebox/port-down.sh");

c8021core::c8021core(const rofl::cdptid &dptid, uint8_t table_id_eth_in,
...
void c8021core::handle_dpt_open() {
  try {
    ...
    rofcore::logging::debug
        << "[c8021core][handle_dpt_open] control channel congested"
        << std::endl;
  } catch (rofl::eRofBaseNotConnected &e) {
    rofcore::logging::debug
        << "[c8021core][handle_dpt_open] control channel not connected"
        << std::endl;
  }
}

void c8021core::handle_dpt_close() {
  try {
    ...
    rofcore::logging::debug
        << "[c8021core][handle_dpt_close] control channel congested"
        << std::endl;
  } catch (rofl::eRofBaseNotConnected &e) {
    rofcore::logging::debug
        << "[c8021core][handle_dpt_close] control channel not connected"
        << std::endl;
  }
}

void c8021core::handle_packet_in(rofl::crofdpt &dpt, const rofl::cauxid &auxid,
                                 rofl::openflow::cofmsg_packet_in &msg) {
  try {
    rofcore::logging::debug
        << "[c8021core][handle_packet_in] pkt received: " << std::endl
        << msg;
    ...
    if (msg.get_table_id() == table_id_eth_src) {
      ...
      if (not msg.get_match().has_vlan_vid()) {
        ...
        rofcore::logging::debug << "[c8021core][handle_packet_in] frame "
                                   "without VLAN tag received, dropping"
                                << std::endl;
    ...
    rofcore::logging::debug
        << "[c8021core][handle_packet_in] no VID match found" << std::endl;
  }
}

void c8021core::handle_flow_removed(rofl::crofdpt &dpt,
  ...
  try {
    ...
    rofcore::logging::debug
        << "[c8021core][handle_flow_removed] no VID match found" << std::endl;
  }
}

void c8021core::handle_port_status(rofl::crofdpt &dpt,
                                   const rofl::cauxid &auxid,
                                   rofl::openflow::cofmsg_port_status &msg) {}

void c8021core::handle_error_message(rofl::crofdpt &dpt,
                                     const rofl::cauxid &auxid,
                                     rofl::openflow::cofmsg_error &msg) {}

void c8021core::enqueue(rofcore::cnetdev *netdev, rofl::cpacket *pkt) {
  try {
    ...
    rofcore::logging::error << "[c8021core][enqueue] no data path attached, "
    ...
    rofcore::logging::error << "[c8021core][enqueue] unable to find tap device"
                            << std::endl;
  }
...
void c8021core::enqueue(rofcore::cnetdev *netdev,
...
void c8021core::hook_port_up(const rofl::crofdpt &dpt,
...
void c8021core::hook_port_down(const rofl::crofdpt &dpt,
                               std::string const &devname) {
  try {
...
void c8021core::execute(std::string const &executable,
                        std::vector<std::string> argv,
                        std::vector<std::string> envp) {
  pid_t pid = 0;
