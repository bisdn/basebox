...
public:
	 ...
	 */
	static c8021core&
	add_8021_core(const rofl::cdptid& dptid,
		...
		c8021core::c8021cores[dptid] = new c8021core(dptid, table_id_eth_in, table_id_eth_src, table_id_eth_local, table_id_eth_dst, default_pvid);
	 ...
	 */
	static c8021core&
	set_8021_core(const rofl::cdptid& dptid,
			uint8_t table_id_eth_in, uint8_t table_id_eth_src,
			uint8_t table_id_eth_local, uint8_t table_id_eth_dst,
			uint16_t default_pvid = DEFAULT_PVID) {
		if (c8021core::c8021cores.find(dptid) == c8021core::c8021cores.end()) {
	 ...
	 *
	 */
	static const c8021core&
	get_8021_core(const rofl::cdptid& dptid) {
		if (c8021core::c8021cores.find(dptid) == c8021core::c8021cores.end()) {
	 ...
	 */
	static void
	drop_8021_core(const rofl::cdptid& dptid) {
		if (c8021core::c8021cores.find(dptid) == c8021core::c8021cores.end()) {
			return;
		}
	 ...
	 */
	static bool
	has_8021_core(const rofl::cdptid& dptid) {
		return (not (c8021core::c8021cores.find(dptid) == c8021core::c8021cores.end()));
	};
...
private:
	 ...
	 *
	 */
	c8021core(const rofl::cdptid& dptid,
			uint8_t table_id_eth_in, uint8_t table_id_eth_src,
			uint8_t table_id_eth_local, uint8_t table_id_eth_dst,
			uint16_t default_pvid = DEFAULT_PVID);
...
public:
	...
	friend std::ostream&
	operator<< (std::ostream& os, const c8021core& core) {
		rofl::RwLock rwlock(core.vlans_rwlock, rofl::RwLock::RWLOCK_READ);
		os << rofcore::indent(0) << "<c8021core default-pvid: " << (int)core.get_default_pvid() << " >" << std::endl;
		rofcore::indent i(2);
