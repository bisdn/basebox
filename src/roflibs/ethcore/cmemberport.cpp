...
void cmemberport::handle_dpt_open(rofl::crofdpt &dpt) {
  try {
...
#endif
    fm.set_instructions().set_inst_goto_table().set_table_id(table_id_eth_in +
                                                             1);
...
void cmemberport::handle_dpt_close(rofl::crofdpt &dpt) {
  try {
    if (rofl::openflow::OFP_VERSION_UNKNOWN == dpt.get_version_negotiated()) {
      return;
    }
...
void cmemberport::handle_packet_in(rofl::crofdpt &dpt,
                                   const rofl::cauxid &auxid,
                                   rofl::openflow::cofmsg_packet_in &msg) {
  // nothing to do
}

void cmemberport::handle_flow_removed(
    rofl::crofdpt &dpt, const rofl::cauxid &auxid,
    rofl::openflow::cofmsg_flow_removed &msg) {
  // nothing to do
}

void cmemberport::handle_port_status(rofl::crofdpt &dpt,
                                     const rofl::cauxid &auxid,
...
void cmemberport::handle_error_message(rofl::crofdpt &dpt,
                                       const rofl::cauxid &auxid,
