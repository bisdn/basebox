...
/*static*/ std::string cethcore::script_path_port_up =
    std::string("/var/lib/basebox/port-up.sh");
/*static*/ std::string cethcore::script_path_port_down =
    std::string("/var/lib/basebox/port-down.sh");

cethcore::cethcore(const rofl::cdptid &dptid, uint8_t table_id_eth_in,
                   uint8_t table_id_eth_src, uint8_t table_id_eth_local,
                   uint8_t table_id_eth_dst, uint16_t default_pvid)
    : state(STATE_IDLE),
      cookie_grp_addr(
          roflibs::common::openflow::ccookie_owner::acquire_cookie()),
      cookie_miss_entry_src(
          roflibs::common::openflow::ccookie_owner::acquire_cookie()),
      cookie_miss_entry_local(
          roflibs::common::openflow::ccookie_owner::acquire_cookie()),
      cookie_redirect_inject(
          roflibs::common::openflow::ccookie_owner::acquire_cookie()),
...
void cethcore::add_eth_endpnts() {
  try {
    ...
    default_pvid =
        portdb.get_default_pvid(rofl::crofdpt::get_dpt(dptid).get_dpid());
    ...
    for (std::set<std::string>::const_iterator it =
             portdb.get_eth_entries(dpid).begin();
         it != portdb.get_eth_entries(dpid).end(); ++it) {
      const cethentry &eth = portdb.get_eth_entry(dpid, *it);

      set_vlan(eth.get_port_vid())
          .add_eth_endpnt(eth.get_hwaddr(), /*tagged=*/false);

      if (not has_tap_dev(dpt.get_dptid(), eth.get_devname())) {
        add_tap_dev(dpt.get_dptid(), eth.get_devname(), eth.get_port_vid(),
                    eth.get_hwaddr());
      }
    }
...
void cethcore::add_phy_ports() {
  try {
    ...
    for (std::map<uint32_t, rofl::openflow::cofport *>::const_iterator it =
             dpt.get_ports().get_ports().begin();
         it != dpt.get_ports().get_ports().end(); ++it) {

      const rofl::openflow::cofport &ofport = *(it->second);
      uint32_t portno = it->first;
      if (not portdb.has_port_entry(rofl::crofdpt::get_dpt(dptid).get_dpid(),
                                    ofport.get_port_no())) {
        set_vlan(default_pvid)
            .add_phy_port(portno, ofport.get_hwaddr(), /*tagged*/ false);
        continue;
      }
      const cportentry &port = portdb.get_port_entry(
          rofl::crofdpt::get_dpt(dptid).get_dpid(), portno);

      // create or update existing vlan for port's default vid
      set_vlan(port.get_port_vid())
          .add_phy_port(portno, ofport.get_hwaddr(), /*tagged=*/false);

      // add all tagged memberships of this port to the appropriate vlan
      for (std::set<uint16_t>::const_iterator jt =
               port.get_tagged_vids().begin();
           jt != port.get_tagged_vids().end(); ++jt) {
        set_vlan(*jt)
            .set_phy_port(portno, ofport.get_hwaddr(), /*tagged=*/true);
      }
    }
  } catch (rofl::eRofDptNotFound &e) {
  }
...
void cethcore::clear_phy_ports() {
  try {
    ...
    // install physical ports
    for (std::map<uint16_t, cvlan>::iterator it = vlans.begin();
         it != vlans.end(); ++it) {
...
void cethcore::handle_dpt_open() {
  try {
    ...
    fm.set_match().set_eth_dst(rofl::caddress_ll("01:80:c2:00:00:00"),
                               rofl::caddress_ll("ff:ff:ff:00:00:00"));
    fm.set_instructions()
    ...
    // forward packets received from special port OFPP_CONTROLLER to
    // table_id_eth_dst
    fm.set_table_id(table_id_eth_in);
    ...
    rofcore::logging::debug
        << "[cethcore][handle_dpt_open] control channel congested" << std::endl;
  } catch (rofl::eRofBaseNotConnected &e) {
    rofcore::logging::debug
        << "[cethcore][handle_dpt_open] control channel not connected"
        << std::endl;
  }
}

void cethcore::handle_dpt_close() {
  try {
    ...
    rofcore::logging::debug
        << "[cethcore][handle_dpt_close] control channel congested"
        << std::endl;
  } catch (rofl::eRofBaseNotConnected &e) {
    rofcore::logging::debug
        << "[cethcore][handle_dpt_close] control channel not connected"
        << std::endl;
  }
}

void cethcore::handle_packet_in(rofl::crofdpt &dpt, const rofl::cauxid &auxid,
                                rofl::openflow::cofmsg_packet_in &msg) {
  try {
    rofcore::logging::debug
        << "[cethcore][handle_packet_in] pkt received: " << std::endl
        << msg;
    ...
    if (msg.get_table_id() == table_id_eth_src) {
      ...
      if (not msg.get_match().has_vlan_vid()) {
        ...
        rofcore::logging::debug << "[cethcore][handle_packet_in] frame without "
                                   "VLAN tag received, dropping"
                                << std::endl;
        dpt.drop_buffer(auxid, msg.get_buffer_id());
        return;
      }
      ...
      if (not msg.get_match().get_eth_dst().is_multicast()) {
        ...
        *pkt = msg.get_packet();
        ...
        for (std::map<std::string, rofcore::ctapdev *>::iterator it =
                 devs[dptid].begin();
             it != devs[dptid].end(); ++it) {
          ...
          *pkt = msg.get_packet();
          ...
          pkt->pop(sizeof(struct rofl::fetherframe::eth_hdr_t) -
                       sizeof(uint16_t),
                   sizeof(struct rofl::fvlanframe::vlan_hdr_t));
          tapdev.enqueue(pkt);
        }
      ...
      // drop buffer on data path, if the packet was stored there, as it is
      // consumed entirely by the underlying kernel
      if (rofl::openflow::base::get_ofp_no_buffer(
              dpt.get_version_negotiated()) != msg.get_buffer_id()) {
        dpt.drop_buffer(auxid, msg.get_buffer_id());
      }
    }

  } catch (rofl::openflow::eOxmNotFound &e) {
    rofcore::logging::debug << "[cethcore][handle_packet_in] no VID match found"
                            << std::endl;
  }
}

void cethcore::handle_flow_removed(rofl::crofdpt &dpt,
  ...
  try {
    ...
    rofcore::logging::debug
        << "[cethcore][handle_flow_removed] no VID match found" << std::endl;
  }
}

void cethcore::handle_port_status(rofl::crofdpt &dpt, const rofl::cauxid &auxid,
  ...
  case rofl::openflow::OFPPR_MODIFY: {
    ...
    if (not portdb.has_port_entry(rofl::crofdpt::get_dpt(dptid).get_dpid(),
      ...
      set_vlan(default_pvid)
          .add_phy_port(msg.get_port().get_port_no(),
                        msg.get_port().get_hwaddr(), /*tagged*/ false);

      // configuration for this port found => add port vid and tagged
      // memberships
    } else {
      const cportentry &port =
          portdb.get_port_entry(rofl::crofdpt::get_dpt(dptid).get_dpid(),
                                msg.get_port().get_port_no());
      ...
      for (std::set<uint16_t>::const_iterator jt =
               port.get_tagged_vids().begin();
           jt != port.get_tagged_vids().end(); ++jt) {
        set_vlan(*jt).set_phy_port(msg.get_port().get_port_no(),
                                   msg.get_port().get_hwaddr(),
                                   /*tagged=*/true);
      }
    }
  ...
  case rofl::openflow::OFPPR_DELETE: {
    ...
    if (not portdb.has_port_entry(rofl::crofdpt::get_dpt(dptid).get_dpid(),
      ...
      // configuration for this port found => drop port vid and tagged
      // memberships
    } else {
      const cportentry &port =
          portdb.get_port_entry(rofl::crofdpt::get_dpt(dptid).get_dpid(),
  ...
  default: {
    ...
    rofcore::logging::error << "[cethcore][handle_port_status] unknown reason"
                            << std::endl;
  }
    return;
  }
...
void cethcore::handle_error_message(rofl::crofdpt &dpt,
                                    const rofl::cauxid &auxid,
...
void cethcore::enqueue(rofcore::cnetdev *netdev, rofl::cpacket *pkt) {
  try {
    ...
    actions.set_action_push_vlan(rofl::cindex(0))
        .set_eth_type(rofl::fvlanframe::VLAN_CTAG_ETHER);
    actions.set_action_set_field(rofl::cindex(1))
        .set_oxm(rofl::openflow::coxmatch_ofb_vlan_vid(tapdev->get_pvid()));
    actions.set_action_output(rofl::cindex(2))
        .set_port_no(rofl::openflow::OFPP_TABLE);
    ...
    rofcore::logging::error
        << "[cethcore][enqueue] no data path attached, dropping outgoing packet"
        << std::endl;
    ...
    rofcore::logging::error << "[cethcore][enqueue] unable to find tap device"
                            << std::endl;
  }
...
void cethcore::enqueue(rofcore::cnetdev *netdev,
...
void cethcore::hook_port_up(const rofl::crofdpt &dpt,
  ...
  try {
    ...
    rofcore::logging::error
        << "[cbasebox][hook_port_up] script execution failed" << std::endl;
  }
}

void cethcore::hook_port_down(const rofl::crofdpt &dpt,
                              std::string const &devname) {
  try {
    ...
    rofcore::logging::error
        << "[cbasebox][hook_port_down] script execution failed" << std::endl;
  }
}

void cethcore::execute(std::string const &executable,
                       std::vector<std::string> argv,
                       std::vector<std::string> envp) {
  pid_t pid = 0;
