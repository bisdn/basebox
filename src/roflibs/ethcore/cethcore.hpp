...
class cethcore : public rofcore::cnetdev_owner,
...
public:
   ...
   *
   */
  static std::set<uint64_t> &get_eth_cores() { return dpids; };

  /**
   *
   */
  static cethcore &
  add_eth_core(const rofl::cdptid &dptid, uint8_t table_id_eth_in,
    ...
    cethcore::ethcores[dptid] =
        new cethcore(dptid, table_id_eth_in, table_id_eth_src,
   ...
   */
  static cethcore &
  set_eth_core(const rofl::cdptid &dptid, uint8_t table_id_eth_in,
               uint8_t table_id_eth_src, uint8_t table_id_eth_local,
               uint8_t table_id_eth_dst, uint16_t default_pvid = DEFAULT_PVID) {
    if (cethcore::ethcores.find(dptid) == cethcore::ethcores.end()) {
      cethcore::ethcores[dptid] =
   ...
   *
   */
  static cethcore &set_eth_core(const rofl::cdptid &dptid) {
    if (cethcore::ethcores.find(dptid) == cethcore::ethcores.end()) {
   ...
   *
   */
  static const cethcore &get_eth_core(const rofl::cdptid &dptid) {
    if (cethcore::ethcores.find(dptid) == cethcore::ethcores.end()) {
   ...
   *
   */
  static void drop_eth_core(const rofl::cdptid &dptid) {
    if (cethcore::ethcores.find(dptid) == cethcore::ethcores.end()) {
      return;
    }
   ...
   *
   */
  static bool has_eth_core(const rofl::cdptid &dptid) {
...
private:
   ...
   *
   */
  cethcore(const rofl::cdptid &dptid, uint8_t table_id_eth_in,
           uint8_t table_id_eth_src, uint8_t table_id_eth_local,
           uint8_t table_id_eth_dst, uint16_t default_pvid = DEFAULT_PVID);
...
public:
   ...
   *
   */
  std::vector<uint16_t> get_vlans() const {
    rofl::RwLock rwlock(vlans_rwlock, rofl::RwLock::RWLOCK_READ);
    std::vector<uint16_t> vids;
    for (std::map<uint16_t, cvlan>::const_iterator it = vlans.begin();
         it != vlans.end(); ++it) {
   ...
   *
   */
  void clear() { vlans.clear(); };

  /**
   *
   */
  cvlan &add_vlan(uint16_t vid) {
    rofl::RwLock rwlock(vlans_rwlock, rofl::RwLock::RWLOCK_WRITE);
    if (vlans.find(vid) != vlans.end()) {
      vlans.erase(vid);
    }
    vlans[vid] = cvlan(dptid, table_id_eth_in, table_id_eth_src,
                       table_id_eth_local, table_id_eth_dst, vid);
   ...
   *
   */
  const cvlan &get_vlan(uint16_t vid) const {
    rofl::RwLock rwlock(vlans_rwlock, rofl::RwLock::RWLOCK_READ);
    if (vlans.find(vid) == vlans.end()) {
   ...
   *
   */
  bool has_vlan(uint16_t vid) const {
    rofl::RwLock rwlock(vlans_rwlock, rofl::RwLock::RWLOCK_READ);
...
public:
  friend std::ostream &operator<<(std::ostream &os, const cethcore &core) {
    rofl::RwLock rwlock(core.vlans_rwlock, rofl::RwLock::RWLOCK_READ);
    os << rofcore::indent(0)
       << "<cethcore default-pvid: " << (int)core.get_default_pvid() << " >"
       << std::endl;
    rofcore::indent i(2);
    ...
    // rofcore::indent i(2);
    os << core.get_dpid();
    for (std::map<uint16_t, cvlan>::const_iterator it = core.vlans.begin();
         it != core.vlans.end(); ++it) {
      os << it->second;
    }
    return os;
  };
