...
public:
   ...
   *
   */
  crpcost(const crpcost &rpcost) { *this = rpcost; };

  /**
   *
   */
  crpcost &operator=(const crpcost &rpcost) {
    if (this == &rpcost)
      return *this;
    rpc = rpcost.rpc;
    return *this;
   ...
   *
   */
  bool operator<(const crpcost &rpcost) const { return (rpc < rpcost.rpc); };
...
public:
  const uint32_t &get_rpcost() const { return rpc; };

public:
  friend std::ostream &operator<<(std::ostream &os, const crpcost &rpcost) {
