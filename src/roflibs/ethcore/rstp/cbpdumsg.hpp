...
public:
   ...
   */
  cbpdumsg()
      : llc_ssap(0), llc_dsap(0), llc_control(0), bpdu_protid(0),
        bpdu_version(0), bpdu_type(0){};

  /**
   * @brief copy constructor
   */
  cbpdumsg(const cbpdumsg &msg) { *this = msg; };

public:
  /**
   * @brief Assignment operator
   */
  cbpdumsg &operator=(const cbpdumsg &bpdu) {
    if (this == &bpdu)
      return *this;
...
public:
   ...
   */
  virtual size_t length() const { return sizeof(bpdu_common_hdr_t); };
...
public:
   ...
   */
  const rofl::caddress_ll &get_eth_src() const { return eth_src; };

  /**
   * @brief Set the Ethernet source address for this bridge PDU.
   */
  void set_eth_src(const rofl::caddress_ll &eth_src) {
   ...
   */
  const rofl::caddress_ll &get_eth_dst() const { return eth_dst; };

  /**
   * @brief Set the Ethernet destination address for this bridge PDU.
   */
  void set_eth_dst(const rofl::caddress_ll &eth_dst) {
   ...
   */
  uint8_t get_llc_ssap() const { return llc_ssap; };

  /**
   * @brief Set LLC SSAP.
   */
  void set_llc_ssap(uint8_t llc_ssap) { this->llc_ssap = llc_ssap; };

  /**
   * @brief Return LLC DSAP.
   */
  uint8_t get_llc_dsap() const { return llc_dsap; };

  /**
   * @brief Set LLC DSAP.
   */
  void set_llc_dsap(uint8_t llc_dsap) { this->llc_dsap = llc_dsap; };

  /**
   * @brief Return LLC control field.
   */
  uint8_t get_llc_control() const { return llc_control; };

  /**
   * @brief Set LLC control field.
   */
  void set_llc_control(uint8_t llc_control) {
   ...
   */
  uint16_t get_bpdu_protid() const { return bpdu_protid; };

  /**
   * @brief Set bridge PDU protocol identifier.
   */
  void set_bpdu_protid(uint16_t bpdu_protid) {
   ...
   */
  uint8_t get_bpdu_version() const { return bpdu_version; };

  /**
   * @brief Set bridge PDU version.
   */
  void set_bpdu_version(uint8_t bpdu_version) {
   ...
   */
  uint8_t get_bpdu_type() const { return bpdu_type; };

  /**
   * @brief Set bridge PDU type.
   */
  void set_bpdu_type(uint8_t bpdu_type) { this->bpdu_type = bpdu_type; };

  /**@}*/

public:
  friend std::ostream &operator<<(std::ostream &os, const cbpdumsg &msg) {
