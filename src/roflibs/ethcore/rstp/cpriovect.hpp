...
public:
	 ...
	 */
	cpriovect(
			const cbridgeid& RootBridgeID,
			const crpcost& RootPathCost,
			const cbridgeid& DesignatedBridgeID,
			const cportid& DesignatedPortID,
			const cportid& BridgePortID) :
...
public:
	...
	// RootBridgeID
	const cbridgeid&
	get_RootBridgeID() const { return RootBridgeID; };

	void
	set_RootBridgeID(const cbridgeid& RootBridgeID) { this->RootBridgeID = RootBridgeID; };

	cbridgeid
	set_RootBridgeID() { return RootBridgeID; };

	// RootPathCost
	const crpcost&
	get_RootPathCost() const { return RootPathCost; };

	void
	set_RootPathCost(const crpcost& RootPathCost) { this->RootPathCost = RootPathCost; };

	crpcost&
	set_RootPathCost() { return RootPathCost; };

	// DesignatedBridgeID
	const cbridgeid&
	get_DesignatedBridgeID() const { return DesignatedBridgeID; };

	void
	set_DesignatedBridgeID(const cbridgeid& DesignatedBridgeID) { this->DesignatedBridgeID = DesignatedBridgeID; };

	cbridgeid&
	set_DesignatedBridgeID() { return DesignatedBridgeID; };

	// DesignatedPortID
	const cportid&
	get_DesignatedPortID() const { return DesignatedPortID; };

	void
	set_DesignatedPortID(const cportid& DesignatedPortID) { this->DesignatedPortID = DesignatedPortID; };

	cportid&
	set_DesignatedPortID() { return DesignatedPortID; };

	// BridgePortID
	const cportid&
	get_BridgePortID() const { return BridgePortID; };

	void
	set_BridgePortID(const cportid& BridgePortID) { this->BridgePortID = BridgePortID; };

	cportid&
	set_BridgePortID() { return BridgePortID; };

public:

	friend std::ostream&
	operator<< (std::ostream& os, const cpriovect& vect) {
