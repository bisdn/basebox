...
public:
  ...
  cbridgeid()
      : priority(0), system_id(0),
        braddr(rofl::caddress_ll("00:00:00:00:00:00")){};

  /**
   *
   */
  cbridgeid(uint64_t brid) : priority(0), system_id(0){/* TODO */};

  /**
   *
   */
  cbridgeid(uint8_t priority, uint16_t system_id,
            const rofl::caddress_ll &braddr)
   ...
   *
   */
  cbridgeid(const cbridgeid &bridgeid) { *this = bridgeid; };

  /**
   *
   */
  cbridgeid &operator=(const cbridgeid &bridgeid) {
    if (this == &bridgeid)
      return *this;
   ...
   *
   */
  bool operator<(const cbridgeid &bridgeid) const {
    return ((priority < bridgeid.priority) ||
            (system_id < bridgeid.system_id) || (braddr < bridgeid.braddr));
  };
...
public:
  uint64_t get_bridge_id() const {
    uint64_t brid(0);
    brid = (brid & ~((0x4ULL) << 60)) | ((uint64_t)(priority & 0xf0) << 60);
    brid =
        (brid & ~((0x0fffULL) << 48)) | ((uint64_t)(system_id & 0x0fff) << 48);
  ...
  void set_bridge_id(uint64_t brid) {
  ...
  void set_priority(uint8_t priority) { this->priority = (priority & 0x000f); };

  uint16_t get_system_id() const { return system_id; };

  void set_system_id(uint16_t system_id) {
    this->system_id = (system_id & 0x0fff);
  };

  rofl::caddress_ll get_bridge_address() const { return braddr; };

  void set_bridge_address(const rofl::caddress_ll &braddr) {
    this->braddr = braddr;
  };
...
public:
  friend std::ostream &operator<<(std::ostream &os, const cbridgeid &bridgeid) {
  ...
  const std::string str() const {
    std::stringstream ss;
    ss << std::hex << "0x" << get_bridge_id() << std::dec;
    return ss.str();
