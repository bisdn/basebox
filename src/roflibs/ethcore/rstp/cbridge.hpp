...
public:
  ...
  virtual void send_bpdu_message(const cportid &portid,
                                 const rofl::cpacket &bpdu) = 0;
};
...
class cbridge : public cport_env, public rofl::ciosrv {
  ...
  cbridge(cbridge_env *env, const cbridgeid &bridgeid)
      : env(env), bridgeid(bridgeid),
        timer_interval_hello_when(DEFAULT_TIMER_INTERVAL_HELLO_WHEN) {
    timerid_hello_when = register_timer(
        TIMER_HELLO_WHEN, rofl::ctimespec(timer_interval_hello_when));
  };
   ...
   *
   */
  cbridge &operator=(const cbridge &bridge);
  // private and never used, not implemented
...
public:
   ...
   *
   */
  static cbridge &add_bridge(cbridge_env *env, const cbridgeid &bridgeid) {
   ...
   *
   */
  static cbridge &set_bridge(cbridge_env *env, const cbridgeid &bridgeid) {
    if (cbridge::bridges.find(bridgeid) == cbridge::bridges.end()) {
      cbridge::bridges[bridgeid] = new cbridge(env, bridgeid);
    }
    return *(cbridge::bridges[bridgeid]);
   ...
   *
   */
  static cbridge &set_bridge(const cbridgeid &bridgeid) {
    if (cbridge::bridges.find(bridgeid) == cbridge::bridges.end()) {
   ...
   *
   */
  static const cbridge &get_bridge(const cbridgeid &bridgeid) {
    if (cbridge::bridges.find(bridgeid) == cbridge::bridges.end()) {
   ...
   *
   */
  static void drop_bridge(const cbridgeid &bridgeid) {
    if (cbridge::bridges.find(bridgeid) == cbridge::bridges.end()) {
      return;
    }
    delete cbridge::bridges[bridgeid];
   ...
   *
   */
  static bool has_bridge(const cbridgeid &bridgeid) {
...
public:
   ...
   *
   */
  cport &add_port(const cportid &portid) {
   ...
   *
   */
  cport &set_port(const cportid &portid) {
    if (ports.find(portid) == ports.end()) {
      ports[portid] = new cport(this, portid);
    }
    return *(ports[portid]);
   ...
   *
   */
  const cport &get_port(const cportid &portid) const {
    if (ports.find(portid) == ports.end()) {
   ...
   *
   */
  void drop_port(const cportid &portid) {
    if (ports.find(portid) == ports.end()) {
      return;
    }
    delete ports[portid];
   ...
   *
   */
  bool has_port(const cportid &portid) const {
    return (not(ports.find(portid) == ports.end()));
  };

public:
  friend std::ostream &operator<<(std::ostream &os, const cbridge &bridge) {
    return os;
  };
...
private:
   ...
   */
  virtual void send_bpdu_message(const cportid &portid,
                                 const rofl::cpacket &bpdu) {
    if (env)
      env->send_bpdu_message(portid, bpdu);
  };

  /**
   *
   */
  void run_engine(enum cbridge_event_t event) {
    events.push_back(event);
    while (not events.empty()) {
      enum cbridge_event_t nxtevent = events.front();
      events.pop_front();
      switch (nxtevent) {
      case EVENT_HELLO_WHEN:
        event_hello_when();
        break;
