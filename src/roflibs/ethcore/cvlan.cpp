...
void cvlan::handle_dpt_open(rofl::crofdpt &dpt) {
  try {
    ...
    fm.set_match().set_eth_dst(rofl::caddress_ll("01:00:00:00:00:00"),
                               rofl::caddress_ll("01:00:00:00:00:00"));
    fm.set_instructions()
        .set_inst_apply_actions()
        .set_actions()
        .add_action_output(rofl::cindex(1))
        .set_port_no(rofl::openflow::OFPP_CONTROLLER);
    fm.set_instructions()
        .set_inst_apply_actions()
        .set_actions()
        .set_action_output(rofl::cindex(1))
        .set_max_len(1526);
    // fm.set_instructions().set_inst_goto_table().set_table_id(table_id_eth_local);
    dpt.send_flow_mod_message(rofl::cauxid(0), fm);

    // send notification to all ethernet endpoints
    for (std::map<rofl::caddress_ll, cethendpnt>::iterator it =
             ethendpnts.begin();
    ...
    for (std::map<uint32_t, cmemberport>::iterator it = phyports.begin();
         it != phyports.end(); ++it) {
      it->second.handle_dpt_open(dpt);
    }

    // send notification to all fib entries
    for (std::map<rofl::caddress_ll, cfibentry>::iterator it = fib.begin();
         it != fib.end(); ++it) {
      it->second.handle_dpt_open(dpt);
    }
    ...
    rofcore::logging::debug
        << "[cvlan][handle_dpt_open] control channel congested" << std::endl;
  } catch (rofl::eRofBaseNotConnected &e) {
    rofcore::logging::debug
        << "[cvlan][handle_dpt_open] control channel not connected"
        << std::endl;
  }
}

void cvlan::handle_dpt_close(rofl::crofdpt &dpt) {
  try {
    // send notification to all fib entries
    for (std::map<rofl::caddress_ll, cfibentry>::iterator it = fib.begin();
         it != fib.end(); ++it) {
    ...
    rofcore::logging::debug
        << "[cvlan][handle_dpt_close] control channel congested" << std::endl;
  } catch (rofl::eRofBaseNotConnected &e) {
    rofcore::logging::debug
        << "[cvlan][handle_dpt_close] control channel not connected"
        << std::endl;
  } catch (...) {
  }

  state = STATE_DETACHED;
  dpt.release_group_id(group_id);
  group_id = 0;
}

void cvlan::handle_packet_in(rofl::crofdpt &dpt, const rofl::cauxid &auxid,
  ...
  if (msg.get_table_id() == table_id_eth_src) {
    try {
      ...
      if (lladdr.is_multicast() || lladdr.is_null()) {
        rofcore::logging::debug
            << "[cvlan][handle_packet_in] invalid source lladdr found"
            << std::endl
            << msg;
      ...
      rofcore::logging::debug << "[cvlan][handle_packet_in] match(es) not found"
                              << std::endl
                              << msg;
      dpt.drop_buffer(auxid, msg.get_buffer_id());
    } catch (eFibEntryPortNotMember &e) {
      rofcore::logging::debug
          << "[cvlan][handle_packet_in] packet-in on invalid port" << std::endl
          << msg;
      dpt.drop_buffer(auxid, msg.get_buffer_id());
    }

  } else if (msg.get_table_id() == table_id_eth_local) {
    rofcore::logging::debug
        << "[cvlan][handle_packet_in] pkt received: " << std::endl
        << msg;
...
void cvlan::handle_flow_removed(rofl::crofdpt &dpt, const rofl::cauxid &auxid,
  ...
  try {
    ...
    if (lladdr.is_multicast() || lladdr.is_null()) {
      rofcore::logging::debug
          << "[cvlan][handle_flow_removed] invalid source lladdr found"
          << std::endl
          << msg;
      return;
    }
    ...
    if (has_fib_entry(lladdr)) {
      set_fib_entry(lladdr)
          .handle_flow_removed(dpt, auxid, msg); // notify fib entry
    }

  } catch (rofl::openflow::eOxmNotFound &e) {
    rofcore::logging::debug
        << "[cvlan][handle_flow_removed] match(es) not found" << std::endl
        << msg;
  }
}

void cvlan::handle_port_status(rofl::crofdpt &dpt, const rofl::cauxid &auxid,
  ...
  try {
    ...
    if (not has_phy_port(portno)) {
      rofcore::logging::debug
          << "[cvlan][handle_port_status] no entry for portno:"
          << (unsigned int)portno << " found in portdb" << std::endl
          << msg;
      return;
    }
    ...
    rofcore::logging::debug << "[cvlan][handle_port_status] port not found"
                            << std::endl
                            << msg;
  }
}

void cvlan::handle_error_message(rofl::crofdpt &dpt, const rofl::cauxid &auxid,
                                 rofl::openflow::cofmsg_error &msg) {
  // TODO
}

void cvlan::update_group_entry_buckets(uint16_t command) {
  try {
    if (STATE_ATTACHED != state) {
      return;
    }

    if (rofl::openflow::OFP_VERSION_UNKNOWN ==
        rofl::crofdpt::get_dpt(dptid).get_version_negotiated()) {
      return;
    }
    ...
    for (std::map<uint32_t, cmemberport>::iterator it = phyports.begin();
      ...
      gm.set_buckets()
          ...
          .set_port_no(it->second.get_port_no());
      bucket_id++;
    }
    ...
    rofcore::logging::debug
        << "[cvlan][update_group_entry_buckets] control channel congested"
        << std::endl;
  } catch (rofl::eRofSockTxAgain &e) {
    rofcore::logging::debug
        << "[cvlan][update_group_entry_buckets] control channel congested"
    ...
    rofcore::logging::debug
        << "[cvlan][update_group_entry_buckets] control channel not connected"
        << std::endl;
  } catch (rofl::eRofDptNotFound &e) {
    rofcore::logging::debug
        << "[cvlan][update_group_entry_buckets] dpt not found" << std::endl;
  }
}
