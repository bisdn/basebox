...
public:
   ...
   *
   */
  cportentry(uint32_t portno = 0, uint16_t port_vid = 1)
      : portno(portno), port_vid(port_vid){};
   ...
   *
   */
  cportentry(const cportentry &port) { *this = port; };

  /**
   *
   */
  cportentry &operator=(const cportentry &port) {
    if (this == &port)
      return *this;
    portno = port.portno;
    port_vid = port.port_vid;
    tagged_vids.clear();
    for (std::set<uint16_t>::const_iterator it = port.tagged_vids.begin();
         it != port.tagged_vids.end(); ++it) {
...
public:
   ...
   *
   */
  void set_port_vid(uint16_t port_vid) { this->port_vid = port_vid; };

  /**
   *
   */
  uint16_t get_port_vid() const { return port_vid; };

  /**
   *
   */
  const std::set<uint16_t> &get_tagged_vids() const { return tagged_vids; };

  /**
   *
   */
  void add_vid(uint16_t vid) { tagged_vids.insert(vid); };

  /**
   *
   */
  void drop_vid(uint16_t vid) { tagged_vids.erase(vid); };

  /**
   *
   */
  void clear_vids() { tagged_vids.clear(); };

public:
  friend std::ostream &operator<<(std::ostream &os, const cportentry &port) {
...
public:
  ...
  cethentry(
      ...
      uint16_t port_vid = 1)
      : devname(devname), hwaddr(hwaddr), port_vid(port_vid){};
   ...
   *
   */
  cethentry(const cethentry &port) { *this = port; };

  /**
   *
   */
  cethentry &operator=(const cethentry &port) {
    if (this == &port)
      return *this;
...
public:
  friend std::ostream &operator<<(std::ostream &os, const cethentry &port) {
...
public:
   ...
   *
   */
  static cethcoredb &get_ethcoredb(const std::string &name);
...
public:
   ...
   */
  void set_default_pvid(const rofl::cdpid &dpid, uint16_t default_pvid) {
   ...
   */
  uint16_t get_default_pvid(const rofl::cdpid &dpid) {
   ...
   * @brief	return default pvid assigned to the port for untagged traffic
   */
  uint16_t get_pvid(const rofl::cdpid &dpid, uint32_t portno) {
   ...
   */
  const std::set<uint16_t> &get_vlan_memberships(const rofl::cdpid &dpid,
                                                 uint32_t portno) {
...
public:
   ...
   *
   */
  const std::set<uint32_t> &get_port_entries(const rofl::cdpid &dpid) const {
   ...
   *
   */
  cportentry &add_port_entry(const rofl::cdpid &dpid, uint32_t portno,
                             uint16_t port_vid) {
   ...
   *
   */
  cportentry &set_port_entry(const rofl::cdpid &dpid, uint32_t portno,
   ...
   *
   */
  const cportentry &get_port_entry(const rofl::cdpid &dpid,
   ...
   *
   */
  void drop_port_entry(const rofl::cdpid &dpid, uint32_t portno) {
    if ((portentries.find(dpid) == portentries.end()) ||
        (portentries.at(dpid).find(portno) == portentries.at(dpid).end())) {
      return;
    }
   ...
   *
   */
  bool has_port_entry(const rofl::cdpid &dpid, uint32_t portno) const {
    if (portentries.find(dpid) == portentries.end()) {
      return false;
    }
    return (
        not(portentries.at(dpid).find(portno) == portentries.at(dpid).end()));
  };
...
public:
   ...
   *
   */
  const std::set<std::string> &get_eth_entries(const rofl::cdpid &dpid) const {
   ...
   *
   */
  cethentry &add_eth_entry(const rofl::cdpid &dpid, const std::string &devname,
                           const rofl::caddress_ll &hwaddr, uint16_t port_vid) {
   ...
   *
   */
  cethentry &set_eth_entry(const rofl::cdpid &dpid, const std::string &devname,
                           const rofl::caddress_ll &hwaddr, uint16_t port_vid) {
    if (ethentries[dpid].find(devname) == ethentries[dpid].end()) {
   ...
   *
   */
  cethentry &set_eth_entry(const rofl::cdpid &dpid,
                           const std::string &devname) {
    if (ethentries[dpid].find(devname) == ethentries[dpid].end()) {
      (void)ethentries[dpid][devname];
    }
   ...
   *
   */
  const cethentry &get_eth_entry(const rofl::cdpid &dpid,
   ...
   *
   */
  void drop_eth_entry(const rofl::cdpid &dpid, const std::string &devname) {
    if ((ethentries.find(dpid) == ethentries.end()) ||
        (ethentries.at(dpid).find(devname) == ethentries.at(dpid).end())) {
      return;
    }
   ...
   *
   */
  bool has_eth_entry(const rofl::cdpid &dpid,
                     const std::string &devname) const {
    if (ethentries.find(dpid) == ethentries.end()) {
      return false;
    }
    return (
        not(ethentries.at(dpid).find(devname) == ethentries.at(dpid).end()));
  };

public:
  friend std::ostream &operator<<(std::ostream &os, const cethcoredb &portdb) {
    os << rofcore::indent(0) << "<cethcoredb >" << std::endl;
    rofcore::indent i(2);
    for (std::map<rofl::cdpid, std::map<uint32_t, cportentry>>::const_iterator
             it = portdb.portentries.begin();
         it != portdb.portentries.end(); ++it) {
      os << rofcore::indent(0) << "<cdpid: " << it->first.str() << " >"
         << std::endl;
      rofcore::indent j(2);
      for (std::map<uint32_t, cportentry>::const_iterator jt =
               it->second.begin();
           jt != it->second.end(); ++jt) {
        os << jt->second;
      }
    ...
    for (std::map<rofl::cdpid, std::map<std::string, cethentry>>::const_iterator
             it = portdb.ethentries.begin();
         it != portdb.ethentries.end(); ++it) {
      os << rofcore::indent(0) << "<cdpid: " << it->first.str() << " >"
         << std::endl;
      rofcore::indent j(2);
      for (std::map<std::string, cethentry>::const_iterator jt =
               it->second.begin();
           jt != it->second.end(); ++jt) {
        os << jt->second;
      }
