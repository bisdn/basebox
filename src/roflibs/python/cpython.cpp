...
std::string cpython::DEFAULT_SCRIPT =
    std::string("/usr/local/sbin/baseboxd.py");

void cpython::run(const std::string &script, const std::string &program_name) {
  if (tid != 0) {
    stop();
  }
  ...
  if ((rc = pthread_create(&tid, NULL, cpython::run_script, NULL)) < 0) {
    throw ePythonFailed(
        "cpython::run() unable to create thread for python interpreter");
  }
...
/*static*/
void *cpython::run_script(void *arg) {
  cpython &p = cpython::get_instance();

  // set program name
  Py_SetProgramName((char *)const_cast<char *>(
      p.get_program_name().c_str())); /* optional but recommended */
