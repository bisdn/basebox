...
public:
   ...
   *
   */
  static cipcore &add_ip_core(const rofl::cdptid &dptid,
    ...
    cipcore::ipcores[dptid] =
        new cipcore(dptid, local_ofp_table_id, out_ofp_table_id);
   ...
   *
   */
  static cipcore &set_ip_core(const rofl::cdptid &dptid,
                              uint8_t local_ofp_table_id,
                              uint8_t out_ofp_table_id) {
    if (cipcore::ipcores.find(dptid) == cipcore::ipcores.end()) {
      cipcore::ipcores[dptid] =
   ...
   *
   */
  static const cipcore &get_ip_core(const rofl::cdptid &dptid) {
    if (cipcore::ipcores.find(dptid) == cipcore::ipcores.end()) {
   ...
   *
   */
  static void drop_ip_core(const rofl::cdptid &dptid) {
    if (cipcore::ipcores.find(dptid) == cipcore::ipcores.end()) {
      return;
    }
    delete cipcore::ipcores[dptid];
   ...
   *
   */
  static bool has_ip_core(const rofl::cdptid &dptid) {
...
private:
   ...
   *
   */
  cipcore(const rofl::cdptid &dptid, uint8_t local_ofp_table_id = 3,
      ...
      : state(STATE_DETACHED), dptid(dptid),
        ...
        cookie_no_route(
            roflibs::common::openflow::ccookie_owner::acquire_cookie()),
        cookie_multicast_ipv4(
            roflibs::common::openflow::ccookie_owner::acquire_cookie()),
        cookie_multicast_ipv6(
            roflibs::common::openflow::ccookie_owner::acquire_cookie()),
        local_ofp_table_id(local_ofp_table_id),
        out_ofp_table_id(out_ofp_table_id), config_file(DEFAULT_CONFIG_FILE){};
...
public:
   ...
   *
   */
  clink &add_link(int ifindex, const std::string &devname,
                  const rofl::caddress_ll &hwaddr, bool tagged = false,
                  uint16_t vid = 1) {
    ...
    links[ifindex] =
        new clink(dptid, ifindex, devname, hwaddr, local_ofp_table_id,
                  out_ofp_table_id, tagged, vid);
   ...
   *
   */
  clink &set_link(int ifindex, const std::string &devname,
                  const rofl::caddress_ll &hwaddr, bool tagged = false,
                  uint16_t vid = 1) {
    if (links.find(ifindex) == links.end()) {
      links[ifindex] =
   ...
   *
   */
  const clink &get_link(int ifindex) const {
    if (links.find(ifindex) == links.end()) {
   ...
   *
   */
  bool has_link(int ifindex) const {
   ...
   *
   */
  const clink &get_link(const std::string &devname) const {
...
public:
   ...
   *
   */
  void clear_routes() { rtables.clear(); };

  /**
   *
   */
  croutetable &add_table(unsigned int rttblid) {
    if (rtables.find(rttblid) != rtables.end()) {
      rtables.erase(rttblid);
    }
    rtables[rttblid] =
        croutetable(rttblid, dptid, local_ofp_table_id, out_ofp_table_id);
    if (STATE_ATTACHED == state) {
   ...
   *
   */
  croutetable &set_table(unsigned int rttblid) {
    if (rtables.find(rttblid) == rtables.end()) {
      rtables[rttblid] =
          croutetable(rttblid, dptid, local_ofp_table_id, out_ofp_table_id);
      if (STATE_ATTACHED == state) {
   ...
   *
   */
  const croutetable &get_table(unsigned int rttblid) const {
    if (rtables.find(rttblid) == rtables.end()) {
   ...
   *
   */
  void drop_table(unsigned int rttblid) {
    if (rtables.find(rttblid) == rtables.end()) {
      return;
    }
    if (STATE_ATTACHED == state) {
   ...
   *
   */
  bool has_table(unsigned int table_id) const {
    return (not(rtables.find(table_id) == rtables.end()));
  };

public:
  friend std::ostream &operator<<(std::ostream &os, const cipcore &ipcore) {
    try {
      os << rofcore::indent(0) << "<cipcore dpid: "
         << rofl::crofdpt::get_dpt(ipcore.dptid).get_dpid().str() << " >"
         << std::endl;
    ...
    rofcore::indent i(2);
    for (std::map<int, clink *>::const_iterator it = ipcore.links.begin();
         it != ipcore.links.end(); ++it) {
      os << *(it->second);
    }
    for (std::map<unsigned int, croutetable>::const_iterator it =
             ipcore.rtables.begin();
         it != ipcore.rtables.end(); ++it) {
      os << it->second;
    }
    return os;
  };
...
private:
  ...
  rofl::cdptid dptid;
  uint64_t
      cookie_fwd_local; // forwarding enabled flow table entry in local table
...
private:
  ...
  void purge_dpt_entries();

  void redirect_ipv4_multicast();

  void redirect_ipv6_multicast();

public:
  virtual void handle_dpt_open();

  virtual void handle_dpt_close();
