...
#include <roflibs/ipcore/caddr.hpp>
...
class clink : public rofcore::cnetlink_common_observer {
public:
  /**
   *
   */
   ...
   *
   */
  clink(const rofl::cdptid &dptid, int ifindex, const std::string &devname,
        const rofl::caddress_ll &hwaddr, uint8_t in_ofp_table_id,
        uint8_t out_ofp_table_id, bool tagged = false, uint16_t vid = 0xffff);
...
public:
   ...
   *
   */
  void set_dptid(const rofl::cdptid &dptid) {
    this->dptid = dptid;
   ...
   *
   */
  rofl::cmacaddr get_hwaddr() const { return hwaddr; };
   ...
   *
   */
  bool get_vlan_tagged() const { return tagged; };

  /**
   *
   */
  uint16_t get_vlan_vid() const { return vid; };
...
public:
   ...
   *
   */
  caddr_in4 &add_addr_in4(unsigned int adindex) {
    if (addrs_in4.find(adindex) != addrs_in4.end()) {
      addrs_in4.erase(adindex);
    }
    addrs_in4[adindex] =
        new caddr_in4(ifindex, adindex, dptid, in_ofp_table_id);
   ...
   *
   */
  caddr_in4 &set_addr_in4(unsigned int adindex) {
    if (addrs_in4.find(adindex) == addrs_in4.end()) {
      addrs_in4[adindex] =
          new caddr_in4(ifindex, adindex, dptid, in_ofp_table_id);
      if (STATE_ATTACHED == state) {
   ...
   *
   */
  const caddr_in4 &get_addr_in4(unsigned int adindex) const {
    if (addrs_in4.find(adindex) == addrs_in4.end()) {
   ...
   *
   */
  void drop_addr_in4(unsigned int adindex) {
    if (addrs_in4.find(adindex) == addrs_in4.end()) {
      return;
    }
   ...
   *
   */
  bool has_addr_in4(unsigned int adindex) const {
   ...
   *
   */
  caddr_in6 &add_addr_in6(unsigned int adindex) {
    if (addrs_in6.find(adindex) != addrs_in6.end()) {
      addrs_in6.erase(adindex);
    }
    addrs_in6[adindex] =
        new caddr_in6(ifindex, adindex, dptid, in_ofp_table_id);
    if (STATE_ATTACHED == state) {
   ...
   *
   */
  caddr_in6 &set_addr_in6(unsigned int adindex) {
    if (addrs_in6.find(adindex) == addrs_in6.end()) {
      addrs_in6[adindex] =
          new caddr_in6(ifindex, adindex, dptid, in_ofp_table_id);
      if (STATE_ATTACHED == state) {
   ...
   *
   */
  const caddr_in6 &get_addr_in6(unsigned int adindex) const {
    if (addrs_in6.find(adindex) == addrs_in6.end()) {
   ...
   *
   */
  void drop_addr_in6(unsigned int adindex) {
    if (addrs_in6.find(adindex) == addrs_in6.end()) {
      return;
    }
   ...
   *
   */
  bool has_addr_in6(unsigned int adindex) const {
...
public:
   ...
   *
   */
  cneigh_in4 &add_neigh_in4(unsigned int nbindex) {
    if (neighs_in4.find(nbindex) != neighs_in4.end()) {
      neighs_in4.erase(nbindex);
    }
    neighs_in4[nbindex] =
        new cneigh_in4(ifindex, nbindex, dptid, out_ofp_table_id);
    if (STATE_ATTACHED == state) {
   ...
   *
   */
  cneigh_in4 &set_neigh_in4(unsigned int nbindex) {
    if (neighs_in4.find(nbindex) == neighs_in4.end()) {
      neighs_in4[nbindex] =
          new cneigh_in4(ifindex, nbindex, dptid, out_ofp_table_id);
      if (STATE_ATTACHED == state) {
   ...
   *
   */
  const cneigh_in4 &get_neigh_in4(unsigned int nbindex) const {
    if (neighs_in4.find(nbindex) == neighs_in4.end()) {
   ...
   *
   */
  void drop_neigh_in4(unsigned int nbindex) {
    if (neighs_in4.find(nbindex) == neighs_in4.end()) {
      return;
    }
   ...
   *
   */
  bool has_neigh_in4(unsigned int nbindex) const {
   ...
   *
   */
  const cneigh_in4 &get_neigh_in4(const rofl::caddress_in4 &dst) const {
   ...
   *
   */
  cneigh_in6 &add_neigh_in6(unsigned int nbindex) {
    if (neighs_in6.find(nbindex) != neighs_in6.end()) {
      neighs_in6.erase(nbindex);
    }
    neighs_in6[nbindex] =
        new cneigh_in6(ifindex, nbindex, dptid, out_ofp_table_id);
    if (STATE_ATTACHED == state) {
   ...
   *
   */
  cneigh_in6 &set_neigh_in6(unsigned int nbindex) {
    if (neighs_in6.find(nbindex) == neighs_in6.end()) {
      neighs_in6[nbindex] =
          new cneigh_in6(ifindex, nbindex, dptid, out_ofp_table_id);
      if (STATE_ATTACHED == state) {
   ...
   *
   */
  const cneigh_in6 &get_neigh_in6(unsigned int nbindex) const {
    if (neighs_in6.find(nbindex) == neighs_in6.end()) {
      throw eNeighNotFound("clink::get_neigh_in4() nbindex not found");
    }
   ...
   *
   */
  void drop_neigh_in6(unsigned int nbindex) {
    if (neighs_in6.find(nbindex) == neighs_in6.end()) {
      return;
    }
   ...
   *
   */
  bool has_neigh_in6(unsigned int nbindex) const {
   ...
   *
   */
  const cneigh_in6 &get_neigh_in6(const rofl::caddress_in6 &dst) const {
...
public:
   ...
   *
   */
  friend std::ostream &operator<<(std::ostream &os, clink const &link) {
    os << rofcore::indent(0) << "<clink: "
       << "ifindex:" << (int)link.ifindex << " "
       << "devname:" << link.get_devname() << " "
       << "vlan:{vid: " << (int)link.vid
       << ", tagged:" << (link.tagged ? "true" : "false") << "} "
       << " >" << std::endl;
...
#endif
    rofcore::indent i(2);
    for (std::map<unsigned int, caddr_in4 *>::const_iterator it =
             link.addrs_in4.begin();
         it != link.addrs_in4.end(); ++it) {
      os << *(it->second);
    }
    for (std::map<unsigned int, caddr_in6 *>::const_iterator it =
             link.addrs_in6.begin();
         it != link.addrs_in6.end(); ++it) {
      os << *(it->second);
    }
    for (std::map<unsigned int, cneigh_in4 *>::const_iterator it =
             link.neighs_in4.begin();
         it != link.neighs_in4.end(); ++it) {
      os << *(it->second);
    }
    for (std::map<unsigned int, cneigh_in6 *>::const_iterator it =
             link.neighs_in6.begin();
         it != link.neighs_in6.end(); ++it) {
      os << *(it->second);
    }
    return os;
  };
