...
#include <roflibs/ipcore/cnexthop.hpp>
...
public:
   ...
   *
   */
  croute(uint8_t rttblid, unsigned int rtindex, const rofl::cdptid &dptid,
  ...
  croute &operator=(const croute &route) {
    ...
    clear();
    for (std::map<uint32_t, cnexthop_in4>::const_iterator it =
             route.nexthops_in4.begin();
         it != route.nexthops_in4.end(); ++it) {
      add_nexthop_in4(it->first) = it->second;
    }
    for (std::map<uint32_t, cnexthop_in6>::const_iterator it =
             route.nexthops_in6.begin();
         it != route.nexthops_in6.end(); ++it) {
  ...
  virtual ~croute() {
    if (STATE_ATTACHED == state) {
      for (std::map<uint32_t, cnexthop_in4>::iterator it = nexthops_in4.begin();
           it != nexthops_in4.end(); ++it) {
        it->second.handle_dpt_close();
      }
      for (std::map<uint32_t, cnexthop_in6>::iterator it = nexthops_in6.begin();
...
public:
   ...
   *
   */
  const rofl::cdptid &get_dpid() const { return dptid; };
   ...
   *
   */
  uint8_t get_table_id() const { return out_ofp_table_id; };
...
public:
   ...
   *
   */
  cnexthop_in4 &add_nexthop_in4(unsigned int nhindex) {
   ...
   *
   */
  cnexthop_in4 &set_nexthop_in4(unsigned int nhindex) {
    if (nexthops_in4.find(nhindex) == nexthops_in4.end()) {
      // nexthops_in4[nhindex] = cdptnexthop_in4(dpid);
    }
    return nexthops_in4[nhindex];
   ...
   *
   */
  const cnexthop_in4 &get_nexthop_in4(unsigned int nhindex) const {
    if (nexthops_in4.find(nhindex) == nexthops_in4.end()) {
   ...
   *
   */
  void drop_nexthop_in4(unsigned int nhindex) {
    if (nexthops_in4.find(nhindex) == nexthops_in4.end()) {
      return;
    }
    nexthops_in4.erase(nhindex);
   ...
   *
   */
  bool has_nexthop_in4(unsigned int nhindex) const {
   ...
   *
   */
  cnexthop_in6 &add_nexthop_in6(unsigned int nhindex) {
   ...
   *
   */
  cnexthop_in6 &set_nexthop_in6(unsigned int nhindex) {
    if (nexthops_in6.find(nhindex) == nexthops_in6.end()) {
      // nexthops_in6[nhindex].set_dpid(dpid);
    }
    return nexthops_in6[nhindex];
   ...
   *
   */
  const cnexthop_in6 &get_nexthop_in6(unsigned int nhindex) const {
    if (nexthops_in6.find(nhindex) == nexthops_in6.end()) {
   ...
   *
   */
  void drop_nexthop_in6(unsigned int nhindex) {
    if (nexthops_in6.find(nhindex) == nexthops_in6.end()) {
      return;
    }
    nexthops_in6.erase(nhindex);
   ...
   *
   */
  bool has_nexthop_in6(unsigned int nhindex) const {
...
public:
   ...
   *
   */
  friend std::ostream &operator<<(std::ostream &os, croute const &route) {
...
public:
   ...
   *
   */
  croute_in4(uint8_t rttblid, unsigned int rtindex, const rofl::cdptid &dptid,
             uint8_t out_ofp_table_id);

  /**
   *
   */
  croute_in4(const croute_in4 &dptroute) { *this = dptroute; };

  /**
   *
   */
  croute_in4 &operator=(const croute_in4 &dptroute) {
    if (this == &dptroute)
      return *this;
...
public:
  friend std::ostream &operator<<(std::ostream &os, const croute_in4 &route) {
    ...
    try {
      const rofcore::crtroute_in4 &rtroute =
          rofcore::cnetlink::get_instance()
              .get_routes_in4(route.get_rttblid())
              .get_route(route.get_rtindex());
      rofcore::indent i(2);
      os << rtroute;
    ...
    rofcore::indent j(2);
    for (std::map<unsigned int, cnexthop_in4>::const_iterator it =
             route.nexthops_in4.begin();
         it != route.nexthops_in4.end(); ++it) {
      os << it->second;
    }
...
public:
   ...
   *
   */
  croute_in6(uint8_t rttblid, unsigned int rtindex, const rofl::cdptid &dptid,
             uint8_t out_ofp_table_id);

  /**
   *
   */
  croute_in6(const croute_in6 &dptroute) { *this = dptroute; };

  /**
   *
   */
  croute_in6 &operator=(const croute_in6 &dptroute) {
    if (this == &dptroute)
      return *this;
    croute::operator=(dptroute);
    return *this;
...
public:
  friend std::ostream &operator<<(std::ostream &os, const croute_in6 &route) {
    ...
    try {
      const rofcore::crtroute_in6 &rtroute =
          rofcore::cnetlink::get_instance()
              .get_routes_in6(route.get_rttblid())
              .get_route(route.get_rtindex());
      rofcore::indent i(2);
      os << rtroute;
    } catch (rofcore::crtlink::eRtLinkNotFound &e) {
    ...
    rofcore::indent j(2);
    for (std::map<unsigned int, cnexthop_in6>::const_iterator it =
             route.nexthops_in6.begin();
         it != route.nexthops_in6.end(); ++it) {
      os << it->second;
    }
