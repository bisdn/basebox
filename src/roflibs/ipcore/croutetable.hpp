...
#include <roflibs/ipcore/croute.hpp>
...
public:
   ...
   */
  croutetable()
      : state(STATE_DETACHED), rttblid(0), local_ofp_table_id(0),
        out_ofp_table_id(2){};
   ...
   *
   */
  croutetable(const croutetable &rtable) { *this = rtable; };

  /**
   *
   */
  croutetable &operator=(const croutetable &rtable) {
    if (this == &rtable)
      return *this;
    state = rtable.state;
    dptid = rtable.dptid;
    ...
    rib4.clear();
    for (std::map<unsigned int, croute_in4>::const_iterator it =
             rtable.rib4.begin();
         it != rtable.rib4.end(); ++it) {
      set_route_in4(it->first) = rtable.get_route_in4(it->first);
    }
    rib6.clear();
    for (std::map<unsigned int, croute_in6>::const_iterator it =
             rtable.rib6.begin();
         it != rtable.rib6.end(); ++it) {
...
public:
   ...
   *
   */
  void clear_in4() { rib4.clear(); };

  /**
   *
   */
  void clear_in6() { rib6.clear(); };

  /**
   *
   */
  croute_in4 &add_route_in4(unsigned int rtindex) {
    if (rib4.find(rtindex) != rib4.end()) {
      rib4.erase(rtindex);
    }
    rib4[rtindex] = croute_in4(rttblid, rtindex, dptid, out_ofp_table_id);
    if (STATE_ATTACHED == state) {
      rib4[rtindex].handle_dpt_open();
    }
    return rib4[rtindex];
  };
   ...
   *
   */
  croute_in4 &set_route_in4(unsigned int rtindex) {
    if (rib4.find(rtindex) == rib4.end()) {
      rib4[rtindex] = croute_in4(rttblid, rtindex, dptid, out_ofp_table_id);
      if (STATE_ATTACHED == state) {
        rib4[rtindex].handle_dpt_open();
    ...
    return (rib4[rtindex] =
                croute_in4(rttblid, rtindex, dptid, out_ofp_table_id));
  };

  /**
   *
   */
  const croute_in4 &get_route_in4(unsigned int rtindex) const {
    if (rib4.find(rtindex) == rib4.end()) {
   ...
   *
   */
  void drop_route_in4(unsigned int rtindex) {
    if (rib4.find(rtindex) == rib4.end()) {
      return;
    }
    rib4.erase(rtindex);
   ...
   *
   */
  bool has_route_in4(unsigned int rtindex) const {
   ...
   *
   */
  croute_in6 &set_route_in6(unsigned int rtindex) {
    if (rib6.find(rtindex) == rib6.end()) {
      rib6[rtindex] = croute_in6(rttblid, rtindex, dptid, out_ofp_table_id);
      if (STATE_ATTACHED == state) {
        rib6[rtindex].handle_dpt_open();
    ...
    return (rib6[rtindex] =
                croute_in6(rttblid, rtindex, dptid, out_ofp_table_id));
  };

  /**
   *
   */
  const croute_in6 &get_route_in6(unsigned int rtindex) const {
    if (rib6.find(rtindex) == rib6.end()) {
   ...
   *
   */
  void drop_route_in6(unsigned int rtindex) {
    if (rib6.find(rtindex) == rib6.end()) {
      return;
    }
    rib6.erase(rtindex);
   ...
   *
   */
  bool has_route_in6(unsigned int rtindex) const {
...
public:
  ...
  void handle_dpt_open() {
    ...
    for (std::map<unsigned int, croute_in4>::iterator it = rib4.begin();
         it != rib4.end(); ++it) {
      it->second.handle_dpt_open();
    }
    for (std::map<unsigned int, croute_in6>::iterator it = rib6.begin();
         it != rib6.end(); ++it) {
      it->second.handle_dpt_open();
    }
  };
...
public:
  friend std::ostream &operator<<(std::ostream &os, const croutetable &rtable) {
    os << rofcore::indent(0)
       << "<croutetable rttblid: " << (unsigned int)rtable.rttblid << " >"
       << std::endl;
