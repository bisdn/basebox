...
public:
   ...
   *
   */
  crtaddr() : ifindex(0), af(0), prefixlen(0), scope(0), flags(0){};

  /**
   *
   */
  crtaddr(struct rtnl_addr *addr)
      : ifindex(0), af(0), prefixlen(0), scope(0), flags(0) {
    char s_buf[128];
    memset(s_buf, 0, sizeof(s_buf));

    nl_object_get(
        (struct nl_object *)addr); // increment reference counter by one

    if (rtnl_addr_get_label(addr))
      label.assign(rtnl_addr_get_label(
          addr)); // label might be null in case of IPv6 addresses
    ...
    nl_object_put(
        (struct nl_object *)addr); // decrement reference counter by one
  };
   ...
   *
   */
  crtaddr &operator=(const crtaddr &rtaddr) {
    if (this == &rtaddr)
      return *this;
   ...
   *
   */
  bool operator==(const crtaddr &rtaddr) {
    return ((label == rtaddr.label) && (ifindex == rtaddr.ifindex) &&
            (af == rtaddr.af) && (prefixlen == rtaddr.prefixlen) &&
            (scope == rtaddr.scope) && (flags == rtaddr.flags));
  };
...
public:
  friend std::ostream &operator<<(std::ostream &os, crtaddr const &rtaddr) {
    os << rofcore::indent(0) << "<crtaddr label: " << rtaddr.label
       << " ifindex: " << rtaddr.ifindex << " >" << std::endl;
    os << rofcore::indent(2) << "<af: " << rtaddr.get_family_s()
       << " prefixlen: " << rtaddr.prefixlen << " >" << std::endl;
    return os;
  };
...
public:
  crtaddr_find(const crtaddr &rtaddr) : rtaddr(rtaddr){};
  bool operator()(const crtaddr &rta) { return (rtaddr == rta); };
...
public:
   ...
   *
   */
  static crtaddr_in4 &get_addr_in4(unsigned int adindex);
...
public:
   ...
   *
   */
  crtaddr_in4(struct rtnl_addr *addr) : crtaddr(addr) {
    ...
    std::string s_local(
        nl_addr2str(rtnl_addr_get_local(addr), s_buf, sizeof(s_buf)));
    if (s_local != "none") {
    ...
    std::string s_peer(
        nl_addr2str(rtnl_addr_get_peer(addr), s_buf, sizeof(s_buf)));
    if (s_peer != "none") {
    ...
    std::string s_bcast(
        nl_addr2str(rtnl_addr_get_broadcast(addr), s_buf, sizeof(s_buf)));
    if (s_bcast != "none") {
   ...
   *
   */
  bool operator<(const crtaddr_in4 &addr) const {
   ...
   *
   */
  bool operator==(const crtaddr_in4 &rtaddr) {
    return ((crtaddr::operator==(rtaddr)) && (local == rtaddr.local) &&
            (peer == rtaddr.peer) && (bcast == rtaddr.bcast) &&
            (mask == rtaddr.mask));
  };
...
public:
   ...
   *
   */
  const rofl::caddress_in4 &get_local_addr() const { return local; };

  /**
   *
   */
  const rofl::caddress_in4 &get_peer_addr() const { return peer; };

  /**
   *
   */
  const rofl::caddress_in4 &get_broadcast_addr() const { return bcast; };

  /**
   *
   */
  const rofl::caddress_in4 &get_mask() const { return mask; };

public:
  friend std::ostream &operator<<(std::ostream &os, const crtaddr_in4 &rtaddr) {
    os << rofcore::indent(0) << "<crtaddr_in4 >" << std::endl;
    {
      rofcore::indent i(2);
      os << dynamic_cast<const crtaddr &>(rtaddr);
    };
  ...
  std::string str() const {
    /*
            90: ep0: <BROADCAST,MULTICAST,UP,LOWER_UP> mtu 1500 qdisc pfifo_fast
     ...
     */
    std::stringstream ss;
    ss << "addr/inet " << local.str() << "/" << get_prefixlen() << " brd "
       << bcast.str();
    ss << " scope " << get_scope() << " ";
    return ss.str();
  };
  ...
  public:
    crtaddr_in4_find_by_local_addr(const rofl::caddress_in4 &local)
        : local(local){};
    bool operator()(const std::pair<uint16_t, crtaddr_in4> &p) {
...
public:
  crtaddr_in4_find(const crtaddr_in4 &rtaddr) : rtaddr(rtaddr){};
  bool operator()(const crtaddr_in4 &rta) { return (rtaddr == rta); };
...
public:
   ...
   *
   */
  static crtaddr_in6 &get_addr_in6(unsigned int adindex);
...
public:
   ...
   *
   */
  crtaddr_in6(struct rtnl_addr *addr) : crtaddr(addr) {
   ...
   *
   */
  bool operator<(const crtaddr_in6 &addr) const {
    return (local < addr.local);
  };
   ...
   *
   */
  bool operator==(const crtaddr_in6 &rtaddr) {
    return ((crtaddr::operator==(rtaddr)) && (local == rtaddr.local) &&
            (peer == rtaddr.peer) && (bcast == rtaddr.bcast) &&
            (mask == rtaddr.mask));
  };
...
public:
   ...
   *
   */
  const rofl::caddress_in6 &get_local_addr() const { return local; };

  /**
   *
   */
  const rofl::caddress_in6 &get_peer_addr() const { return peer; };

  /**
   *
   */
  const rofl::caddress_in6 &get_broadcast_addr() const { return bcast; };

  /**
   *
   */
  const rofl::caddress_in6 &get_mask() const { return mask; };

public:
  friend std::ostream &operator<<(std::ostream &os, const crtaddr_in6 &rtaddr) {
  ...
  std::string str() const {
     ...
     */
    std::stringstream ss;
    ss << "addr/inet6 " << local.str() << "/" << get_prefixlen() << " brd "
       << bcast.str();
    ss << " scope " << get_scope() << " ";
    return ss.str();
  };
  ...
  public:
    crtaddr_in6_find_by_local_addr(const rofl::caddress_in6 &local)
        : local(local){};
    bool operator()(const std::pair<uint16_t, crtaddr_in6> &p) {
      return (local == p.second.local);
    };
  };
...
public:
  crtaddr_in6_find(const crtaddr_in6 &rtaddr) : rtaddr(rtaddr){};
  bool operator()(const crtaddr_in6 &rta) { return (rtaddr == rta); };
