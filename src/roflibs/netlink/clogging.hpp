...
#ifndef ROFCORE_LOGGING_H_
#define ROFCORE_LOGGING_H_ 1
...
public:
  indent(unsigned int my_width = 0) : my_width(my_width) {
    indent::width += my_width;
  };
  virtual ~indent() {
    indent::width =
        (indent::width >= my_width) ? (indent::width - my_width) : 0;
  };
  static void inc(unsigned int width) { indent::width += width; };
  static std::ostream &inc(std::ostream &os, unsigned int width) {
    indent::width += width;
    return os;
  };
  ...
  static std::ostream &dec(std::ostream &os, unsigned int width) {
    indent::width = (indent::width >= width) ? (indent::width - width) : 0;
    return os;
  };
  static void null() { indent::width = 0; };
  friend std::ostream &operator<<(std::ostream &os, indent const &i) {
    if (indent::width) {
      os << std::setw(indent::width) << " " << std::setw(0);
    }
    return os;
  };
  static unsigned int get_width() { return indent::width; };
};

}; // end of namespace

#endif /* ROFCORE_LOGGING_H_ */
