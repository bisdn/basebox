...
public:
   ...
   */
  crtroute()
      : table_id(0), scope(0), tos(0), protocol(0), priority(0), family(0),
        prefixlen(0), type(0), flags(0), metric(0), iif(0){};
   ...
   *
   */
  crtroute(const crtroute &rtroute) { *this = rtroute; };

  /**
   *
   */
  crtroute &operator=(const crtroute &rtroute) {
    if (this == &rtroute)
      return *this;
  ...
  crtroute(struct rtnl_route *route)
    ...
    metric =
        rtnl_route_get_metric(route, 0, NULL); // FIXME: check the integer value
   ...
   *
   */
  bool operator==(const crtroute &rtroute) {
    // FIXME: anything else beyond this?
    return ((table_id == rtroute.table_id) && (scope == rtroute.scope) &&
            (iif == rtroute.iif));
  };
...
public:
  ...
  std::string get_table_id_s() const {
    ...
    switch (table_id) {
    /*255*/ case RT_TABLE_LOCAL:
      str = std::string("local");
      break;
    /*254*/ case RT_TABLE_MAIN:
      str = std::string("main");
      break;
    /*253*/ case RT_TABLE_DEFAULT:
      str = std::string("default");
      break;
    /*252*/ case RT_TABLE_COMPAT:
      str = std::string("compat");
      break;
    default:
  ...
  std::string get_scope_s() const {
    ...
    switch (scope) {
    /*255*/ case RT_SCOPE_NOWHERE:
      str = std::string("nowhere");
      break;
    /*254*/ case RT_SCOPE_HOST:
      str = std::string("host");
      break;
    /*253*/ case RT_SCOPE_LINK:
      str = std::string("link");
      break;
    /*200*/ case RT_SCOPE_SITE:
      str = std::string("site");
      break;
    /*000*/ case RT_SCOPE_UNIVERSE:
      str = std::string("universe");
      break;
    default:
      str = std::string("unknown");
      break;
   ...
   *
   */
  unsigned int get_metric() const { return metric; };
...
public:
  friend std::ostream &operator<<(std::ostream &os, crtroute const &route) {
    os << rofcore::indent(0) << "<crtroute: >" << std::endl;
    os << rofcore::indent(2) << "<table_id: " << route.get_table_id_s() << " >"
       << std::endl;
    os << rofcore::indent(2) << "<scope: " << route.get_scope_s() << " >"
       << std::endl;
    os << rofcore::indent(2) << "<tos: " << (unsigned int)route.tos << " >"
       << std::endl;
    os << rofcore::indent(2) << "<protocol: " << (unsigned int)route.protocol
       << " >" << std::endl;
    os << rofcore::indent(2) << "<priority: " << (unsigned int)route.priority
       << " >" << std::endl;
    os << rofcore::indent(2) << "<family: " << (unsigned int)route.family
       << " >" << std::endl;
    os << rofcore::indent(2) << "<prefixlen: " << route.prefixlen << " >"
       << std::endl;
    os << rofcore::indent(2) << "<type: " << (unsigned int)route.type << " >"
       << std::endl;
    os << rofcore::indent(2) << "<flags: " << (unsigned int)route.flags << " >"
       << std::endl;
    os << rofcore::indent(2) << "<metric: " << (unsigned int)route.metric
       << " >" << std::endl;
    os << rofcore::indent(2) << "<ifindex: " << (unsigned int)route.iif << " >"
       << std::endl;
...
public:
  crtroute_find(const crtroute &rtroute) : rtroute(rtroute){};
  bool operator()(const crtroute &rta) { return (rtroute == rta); };
...
public:
   ...
   *
   */
  virtual ~crtroute_in4(){};

  /**
   *
   */
  crtroute_in4(const crtroute_in4 &route) { *this = route; };

  /**
   *
   */
  crtroute_in4 &operator=(const crtroute_in4 &rtroute) {
    if (this == &rtroute)
      return *this;
   ...
   *
   */
  crtroute_in4(struct rtnl_route *route) : crtroute(route) {
    ...
    std::string s_dst(
        nl_addr2str(rtnl_route_get_dst(route), s_buf, sizeof(s_buf)));
    if (s_dst != "none") {
    ...
    std::string s_src(
        nl_addr2str(rtnl_route_get_src(route), s_buf, sizeof(s_buf)));
    if (s_src != "none") {
      s_src = s_src.substr(0, s_src.find_first_of("/", 0));
      src = rofl::caddress_in4(s_src);
    }

    std::string s_pref_src(
        nl_addr2str(rtnl_route_get_pref_src(route), s_buf, sizeof(s_buf)));
    if (s_pref_src != "none") {
    ...
    for (int i = 0; i < rtnl_route_get_nnexthops(route); i++) {
      nxthops.add_nexthop(
          crtnexthop_in4(route, rtnl_route_nexthop_n(route, i)));
    }
   ...
   *
   */
  bool operator==(const crtroute_in4 &route) {
    return (crtroute::operator==(route) && (dst == route.dst) &&
            (mask == route.mask) && (src == route.src) &&
...
public:
   ...
   *
   */
  const crtnexthops_in4 &get_nexthops_in4() const { return nxthops; };

  /**
   *
   */
  crtnexthops_in4 &set_nexthops_in4() { return nxthops; };

  /**
   *
   */
  rofl::caddress_in4 &set_ipv4_dst() { return dst; };

  /**
   *
   */
  const rofl::caddress_in4 &get_ipv4_dst() const { return dst; };

  /**
   *
   */
  rofl::caddress_in4 &set_ipv4_mask() { return mask; };

  /**
   *
   */
  const rofl::caddress_in4 &get_ipv4_mask() const { return mask; };

  /**
   *
   */
  rofl::caddress_in4 &set_ipv4_src() { return src; };

  /**
   *
   */
  const rofl::caddress_in4 &get_ipv4_src() const { return src; };

  /**
   *
   */
  rofl::caddress_in4 &set_ipv4_pref_src() { return pref_src; };

  /**
   *
   */
  const rofl::caddress_in4 &get_ipv4_pref_src() const { return pref_src; };

public:
  friend std::ostream &operator<<(std::ostream &os, const crtroute_in4 &route) {
  ...
  std::string str() const {
    std::stringstream ss;
    ss << "route/inet " << dst.str() << "/" << get_prefixlen() << " scope "
       << get_scope_s() << " src " << src.str() << " pref-src "
       << pref_src.str();
    switch (get_scope()) {
  ...
  public:
    crtroute_in4_find(uint32_t table_id, uint8_t scope, unsigned int ifindex,
...
public:
  crtroute_in4_find(const crtroute_in4 &rtroute) : rtroute(rtroute){};
  bool operator()(const crtroute_in4 &rta) { return (rtroute == rta); };
  bool operator()(const std::pair<unsigned int, crtroute_in4> &p) {
    return (rtroute == p.second);
  };
#if 0
...
public:
   ...
   *
   */
  virtual ~crtroute_in6(){};

  /**
   *
   */
  crtroute_in6(const crtroute_in6 &route) { *this = route; };

  /**
   *
   */
  crtroute_in6 &operator=(const crtroute_in6 &rtroute) {
    if (this == &rtroute)
      return *this;
    crtroute::operator=(rtroute);
    dst = rtroute.dst;
   ...
   *
   */
  crtroute_in6(struct rtnl_route *route) : crtroute(route) {
    ...
    for (int i = 0; i < rtnl_route_get_nnexthops(route); i++) {
      nxthops.add_nexthop(
          crtnexthop_in6(route, rtnl_route_nexthop_n(route, i)));
    }
   ...
   *
   */
  bool operator==(const crtroute_in6 &route) {
    return (crtroute::operator==(route) && (dst == route.dst) &&
            (mask == route.mask) && (src == route.src) &&
            (pref_src == route.pref_src));
  };
...
public:
   ...
   *
   */
  const crtnexthops_in6 &get_nexthops_in6() const { return nxthops; };

  /**
   *
   */
  crtnexthops_in6 &set_nexthops_in6() { return nxthops; };

  /**
   *
   */
  rofl::caddress_in6 &set_ipv6_dst() { return dst; };

  /**
   *
   */
  const rofl::caddress_in6 &get_ipv6_dst() const { return dst; };

  /**
   *
   */
  rofl::caddress_in6 &set_ipv6_mask() { return mask; };

  /**
   *
   */
  const rofl::caddress_in6 &get_ipv6_mask() const { return mask; };

  /**
   *
   */
  rofl::caddress_in6 &set_ipv6_src() { return src; };

  /**
   *
   */
  const rofl::caddress_in6 &get_ipv6_src() const { return src; };

  /**
   *
   */
  rofl::caddress_in6 &set_ipv6_pref_src() { return pref_src; };

  /**
   *
   */
  const rofl::caddress_in6 &get_ipv6_pref_src() const { return pref_src; };

public:
  friend std::ostream &operator<<(std::ostream &os, const crtroute_in6 &route) {
  ...
  std::string str() const {
    std::stringstream ss;
    ss << "route/inet6 " << dst.str() << "/" << get_prefixlen() << " scope "
       << get_scope_s() << " src " << src.str() << " pref-src "
       << pref_src.str();
    switch (get_scope()) {
    case RT_SCOPE_UNIVERSE: {
  ...
  public:
    crtroute_in6_find(uint32_t table_id, uint8_t scope, unsigned int ifindex,
...
public:
  crtroute_in6_find(const crtroute_in6 &rtroute) : rtroute(rtroute){};
  bool operator()(const crtroute_in6 &rta) { return (rtroute == rta); };
  bool operator()(const std::pair<unsigned int, crtroute_in6> &p) {
    return (rtroute == p.second);
  };
#if 0
