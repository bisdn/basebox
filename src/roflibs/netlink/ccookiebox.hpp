...
public:
   ...
   *
   */
  virtual ~ccookie_owner();

public:
  /**
   *
   */
  uint64_t acquire_cookie();

  /**
   *
   */
  void release_cookie(uint64_t cookie);
...
public:
  ccookie_find_by_owner(ccookie_owner *owner) : owner(owner){};
...
public:
   ...
   *
   */
  static ccookiebox &get_instance() {
  ...
  void handle_packet_in(rofl::crofdpt &dpt, const rofl::cauxid &auxid,
    ...
    if (cookiestore.find(msg.get_cookie()) == cookiestore.end()) {
      rofcore::logging::debug
          << "[ccookiebox][handle_packet_in] cookie: 0x" << std::hex
          << (unsigned long long)msg.get_cookie() << std::dec
  ...
  void handle_flow_removed(rofl::crofdpt &dpt, const rofl::cauxid &auxid,
    ...
    if (cookiestore.find(msg.get_cookie()) == cookiestore.end()) {
      rofcore::logging::debug
          << "[ccookiebox][handle_flow_removed] cookie: 0x" << std::hex
          << (unsigned long long)msg.get_cookie() << std::dec
          << " not found, dropping packet" << std::endl
          << *this;
      return;
...
private:
   ...
   *
   */
  void deregister_cookie_owner(ccookie_owner *owner) {
   ...
   *
   */
  uint64_t acquire_cookie(ccookie_owner *owner) {
    do {
      next_cookie++;
    ...
    rofcore::logging::debug << "[ccookiebox][acquire_cookie] cookie: 0x"
                            << std::hex << (unsigned long long)next_cookie
                            << std::dec << std::endl;
    return next_cookie;
  };
   ...
   *
   */
  void release_cookie(ccookie_owner *owner, uint64_t cookie) {
    ...
    rofcore::logging::debug << "[ccookiebox][release_cookie] cookie: 0x"
                            << std::hex << (unsigned long long)cookie
                            << std::dec << std::endl;
    cookiestore.erase(cookie);
  };

public:
  friend std::ostream &operator<<(std::ostream &os, const ccookiebox &box) {
    os << rofcore::indent(0) << "<ccookiebox >" << std::endl;
    for (std::map<uint64_t, ccookie_owner *>::const_iterator it =
             box.cookiestore.begin();
         it != box.cookiestore.end(); ++it) {
      os << rofcore::indent(2) << "cookie: 0x" << std::hex
         << (unsigned long long)it->first << std::dec << " => 0x" << std::hex
         << (unsigned long)it->second << std::dec << std::endl;
    }
    return os;
  };
};
