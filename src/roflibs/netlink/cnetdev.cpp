...
void cnetdev_owner::netdev_removed(cnetdev *netdev) {
  if (0 == netdev) {
...
void cnetdev_owner::enqueue(cnetdev *netdev, rofl::cpacket *pkt) {
  if (0 != pkt) {
    cpacketpool::get_instance().release_pkt(pkt);
  }
}

void cnetdev_owner::enqueue(cnetdev *netdev,
...
cnetdev::cnetdev(cnetdev_owner *netdev_owner, std::string const &devname,
                 pthread_t tid)
    : devname(devname), netdev_owner(netdev_owner), ifindex(0) {
  if (0 == netdev_owner) {
...
cnetdev::~cnetdev() { netdev_owner->netdev_removed(this); }

std::string const &cnetdev::get_devname() const { return devname; }

void cnetdev::enqueue(rofl::cpacket *pkt) {
  if (0 != pkt) {
    cpacketpool::get_instance().release_pkt(pkt);
  }
}

void cnetdev::enqueue(std::vector<rofl::cpacket *> pkts) {
  for (std::vector<rofl::cpacket *>::iterator it = pkts.begin();
       it != pkts.end(); ++it) {
    cpacketpool::get_instance().release_pkt(*it);
  }
...
void cnetdev::enable_interface() {
  ...
  if ((rc = ioctl(sd, SIOCGIFINDEX, &ifr)) < 0)
    throw eNetDevIoctl(
        "cnetdev::enable_interface() ioctl() syscall SIOCGIFINDEX failed");

  if ((rc = ioctl(sd, SIOCGIFFLAGS, &ifr)) < 0) {
    close(sd);
    throw eNetDevIoctl(
        "cnetdev::enable_interface() ioctl() syscall SIOCGIFFLAGS failed");
  }
...
void cnetdev::disable_interface() {
  struct ifreq ifr;
  int sd, rc;
  ...
  if ((rc = ioctl(sd, SIOCGIFINDEX, &ifr)) < 0)
    throw eNetDevIoctl(
        "cnetdev::disable_interface() ioctl() syscall SIOCGIFINDEX failed");

  if ((rc = ioctl(sd, SIOCGIFFLAGS, &ifr)) < 0) {
    close(sd);
    throw eNetDevIoctl(
        "cnetdev::disable_interface() ioctl() syscall SIOCGIFFLAGS failed");
  }
  ...
  if ((rc = ioctl(sd, SIOCSIFFLAGS, &ifr)) < 0) {
    close(sd);
    throw eNetDevIoctl(
        "cnetdev::disable_interface() ioctl() syscall SIOCSIFFLAGS failed");
  }

  close(sd);
}

unsigned int cnetdev::get_ifindex() const {
  if (ifindex) {
    return ifindex;
  }
  ...
  if ((rc = ioctl(sd, SIOCGIFINDEX, &ifr)) < 0)
    throw eNetDevIoctl(
        "cnetdev::get_ifindex() ioctl() syscall SIOCGIFINDEX failed");
...
rofl::cmacaddr cnetdev::get_hwaddr() {
  ...
  if ((rc = ioctl(sd, SIOCGIFHWADDR, &ifr)) < 0)
    throw eNetDevIoctl(
        "cnetdev::get_hwaddr() ioctl() syscall SIOCGIFHWADDR failed");
...
void cnetdev::set_hwaddr(rofl::cmacaddr const &hwaddr) {
  // get ifr_ifhwaddr for devname
  int sd, rc;
  if ((sd = socket(AF_PACKET, SOCK_RAW, 0)) < 0)
  ...
  if ((rc = ioctl(sd, SIOCSIFHWADDR, &ifr)) < 0) {
    throw eNetDevIoctl(
        "cnetdev::set_hwaddr() ioctl() syscall SIOCSIFHWADDR failed");
  }
