...
ctundev::ctundev(cnetdev_owner *netdev_owner, std::string const &devname)
    : cnetdev(netdev_owner, devname), fd(-1), devname(devname), thread(this) {
  try {
    tun_open(devname);
  } catch (...) {
...
void ctundev::tun_open(std::string const &devname) {
  try {
    struct ifreq ifr;
    int rc;
    ...
    rofcore::logging::debug
        << "[ctundev][tun_open] exception caught: " << e.what() << std::endl;
    throw;
  }
}

void ctundev::tun_close() {
  try {
    if (fd == -1) {
      return;
    }
    ...
    rofcore::logging::debug
        << "[ctundev][tun_close] exception caught: " << e.what() << std::endl;
  }
...
void ctundev::enqueue(rofl::cpacket *pkt) {
  if (fd == -1) {
...
void ctundev::enqueue(std::vector<rofl::cpacket *> pkts) {
  if (fd == -1) {
...
void ctundev::handle_read_event(rofl::cthread &thread, int fd) {
  ...
  try {
    ...
    if (rc < 0) {
      switch (errno) {
      case EAGAIN:
        throw eNetDevAgain("ctundev::handle_revent() EAGAIN, retrying later");
      default:
        throw eNetDevCritical("ctundev::handle_revent() error occured");
      }
    } else {
    ...
    rofcore::logging::debug << "[ctundev][handle_revent] packet pool "
                               "exhausted, no idle slots available"
                            << std::endl;

  } catch (eNetDevAgain &e) {

    rofcore::logging::debug << "[ctundev][handle_revent] EAGAIN, retrying later"
                            << std::endl;
    ...
    rofcore::logging::debug << "[ctundev][handle_revent] error occured"
                            << std::endl;
...
void ctundev::handle_write_event(rofl::cthread &thread, int fd) {
  rofl::cpacket *pkt = (rofl::cpacket *)0;
  try {
    ...
    while (not pout_queue.empty()) {
      ...
      if ((rc = write(fd, pkt->soframe(), pkt->length())) < 0) {
        ...
        switch (errno) {
        case EAGAIN:
          throw eNetDevAgain("ctundev::handle_wevent() EAGAIN, retrying later");
        default:
          throw eNetDevCritical("ctundev::handle_wevent() error occured");
        }
      }
    ...
    // keep fd in wfds
    rofcore::logging::debug << "[ctundev][handle_wevent] EAGAIN, retrying later"
                            << std::endl;

  } catch (eNetDevCritical &e) {

    rofcore::logging::debug << "[ctundev][handle_wevent] error occured"
                            << std::endl;
...
void ctundev::handle_timeout(rofl::cthread &thread, uint32_t timer_id,
