...
public:
   ...
   *
   */
  static cflowcore &add_flow_core(const rofl::cdptid &dptid) {
   ...
   *
   */
  static cflowcore &set_flow_core(const rofl::cdptid &dptid) {
    if (cflowcore::flowcores.find(dptid) == cflowcore::flowcores.end()) {
      cflowcore::flowcores[dptid] = new cflowcore(dptid);
    }
    return *(cflowcore::flowcores[dptid]);
   ...
   *
   */
  static const cflowcore &get_flow_core(const rofl::cdptid &dptid) {
    if (cflowcore::flowcores.find(dptid) == cflowcore::flowcores.end()) {
   ...
   *
   */
  static void drop_flow_core(const rofl::cdptid &dptid) {
    if (cflowcore::flowcores.find(dptid) == cflowcore::flowcores.end()) {
      return;
    }
    delete cflowcore::flowcores[dptid];
   ...
   *
   */
  static bool has_flow_core(const rofl::cdptid &dptid) {
    return (
        not(cflowcore::flowcores.find(dptid) == cflowcore::flowcores.end()));
  };
...
private:
   ...
   *
   */
  cflowcore(const rofl::cdptid &dptid) : state(STATE_DETACHED), dptid(dptid){};
...
public:
  ...
  void handle_dpt_open() {
    for (std::map<uint32_t, cflow *>::iterator it = flows.begin();
         it != flows.end(); ++it) {
...
public:
   ...
   *
   */
  std::vector<uint32_t> get_svc_flows() const {
    rofl::RwLock rwl(rwlock, rofl::RwLock::RWLOCK_READ);
    std::vector<uint32_t> termids;
    for (std::map<uint32_t, cflow *>::const_iterator it = flows.begin();
         it != flows.end(); ++it) {
   ...
   *
   */
  cflow &add_svc_flow(uint32_t flow_id,
   ...
   *
   */
  cflow &set_svc_flow(uint32_t flow_id,
                      const rofl::openflow::cofflowmod &flowmod) {
    rofl::RwLock rwl(rwlock, rofl::RwLock::RWLOCK_WRITE);
    if (flows.find(flow_id) == flows.end()) {
      flows[flow_id] = new cflow(dptid, flowmod);
   ...
   *
   */
  const cflow &get_svc_flow(uint32_t flow_id) const {
    rofl::RwLock rwl(rwlock, rofl::RwLock::RWLOCK_READ);
    if (flows.find(flow_id) == flows.end()) {
   ...
   *
   */
  void drop_svc_flow(uint32_t flow_id) {
    rofl::RwLock rwl(rwlock, rofl::RwLock::RWLOCK_WRITE);
    if (flows.find(flow_id) == flows.end()) {
      return;
    }
   ...
   *
   */
  bool has_svc_flow(uint32_t flow_id) const {
    rofl::RwLock rwl(rwlock, rofl::RwLock::RWLOCK_READ);
...
public:
  friend std::ostream &operator<<(std::ostream &os, const cflowcore &flowcore) {
