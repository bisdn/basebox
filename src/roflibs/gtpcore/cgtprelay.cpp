...
void cgtprelay::add_gtp_termdevs() {
  try {
    ...
    for (std::set<unsigned int>::const_iterator it =
             db.get_gtp_term_ids(dpid).begin();
         it != db.get_gtp_term_ids(dpid).end(); ++it) {
...
void cgtprelay::handle_read(rofl::csocket &socket) {
  ...
  try {
    ...
    case AF_INET: {
      try {
        ...
        roflibs::gtp::caddress_gtp_in4 gtp_dst_addr(
            rofl::caddress_in4(socket.get_laddr().ca_s4addr,
                               sizeof(struct sockaddr_in)),
            roflibs::gtp::cport(socket.get_laddr().ca_s4addr,
                                sizeof(struct sockaddr_in)));

        roflibs::gtp::clabel_in4 label_in(gtp_src_addr, gtp_dst_addr,
                                          roflibs::gtp::cteid(gtpu.get_teid()));

        rofcore::logging::debug
            << "[cgtprelay][handle_read] label-incoming/label-egress: "
            << std::endl
            << label_in;
        ...
        if (cgtpcore::get_gtp_core(dptid).has_relay_in4(label_in)) {
          ...
          rofcore::logging::debug
              << "[cgtprelay][handle_read][relay] found relay: " << std::endl
              << relay;
          ...
          // forward GTP message
          set_socket_in4(label_out.get_saddr())
              .send(mem, rofl::csockaddr(
                             label_out.get_daddr().get_addr(),
                             label_out.get_daddr().get_port().get_value()));

          // set OFP shortcut into datapath
          cgtpcore::set_gtp_core(dptid)
              .set_relay_in4(label_in)
              .handle_dpt_open();

        } else
            /* this is the incoming label == egress label */
            if (cgtpcore::get_gtp_core(dptid)
                    .has_term_in4(/*egress label*/ label_in)) {
          ...
          rofcore::logging::debug
              << "[cgtprelay][handle_read][term] found termination point: "
              << std::endl
              << term;

          rofl::cpacket *pkt =
              rofcore::cpacketpool::get_instance().acquire_pkt();
          ...
          rofcore::logging::debug
              << "[cgtprelay][handle_read] no relay or termination point found"
              << std::endl;
        }

      } catch (eGtpCoreNotFound &e) {
        rofcore::logging::debug << "[cgtprelay][handle_read] gtpcore not found"
                                << std::endl;
        delete mem;
      } catch (eRelayNotFound &e) {
        rofcore::logging::debug
            << "[cgtprelay][handle_read] relay_in4 not found" << std::endl;
        delete mem;
      } catch (eTermNotFound &e) {
        rofcore::logging::debug << "[cgtprelay][handle_read] term_in4 not found"
                                << std::endl;
        delete mem;
      }
    } break;
    case AF_INET6: {
      try {
        ...
        roflibs::gtp::caddress_gtp_in6 gtp_dst_addr(
            rofl::caddress_in6(socket.get_laddr().ca_s6addr,
                               sizeof(struct sockaddr_in6)),
            roflibs::gtp::cport(socket.get_laddr().ca_s6addr,
                                sizeof(struct sockaddr_in6)));

        roflibs::gtp::clabel_in6 label_in(gtp_src_addr, gtp_dst_addr,
                                          roflibs::gtp::cteid(gtpu.get_teid()));
        ...
        if (cgtpcore::get_gtp_core(dptid).has_relay_in6(label_in)) {
          ...
          // forward GTP message
          set_socket_in6(label_out.get_saddr())
              .send(mem, rofl::csockaddr(
                             label_out.get_daddr().get_addr(),
                             label_out.get_daddr().get_port().get_value()));
        ...
        rofcore::logging::debug
            << "[cgtprelay][handle_read] relay_in6 not found" << std::endl;
        delete mem;
      } catch (eTermNotFound &e) {
        rofcore::logging::debug << "[cgtprelay][handle_read] term_in6 not found"
                                << std::endl;
        delete mem;
      }
    } break;
    ...
    rofcore::logging::debug << "[cgtprelay][handle_read] eSocketRxAgain: no "
                               "further data available on socket"
                            << std::endl;
    delete mem;
  } catch (rofl::eSysCall &e) {
    rofcore::logging::warn
        << "[cgtprelay][handle_read] failed to read from socket: " << e
        << std::endl;
    delete mem;
  } catch (rofl::RoflException &e) {
    rofcore::logging::warn
        << "[cgtprelay][handle_read] dropping invalid message: " << e
        << std::endl;
    delete mem;
  }
}

void cgtprelay::enqueue(rofcore::cnetdev *netdev,
...
void cgtprelay::enqueue(rofcore::cnetdev *netdev, rofl::cpacket *pkt) {
  rofcore::logging::debug << "[rofgtp][cgtprelay][enqueue] msg received: "
                          << std::endl
                          << *pkt;

  // read the first byte (to be precise: the first nibble) of this packet and
  ...
  switch (ip_version) {
  case IP_VERSION_4:
    enqueue_in4(netdev, pkt);
    break;
  case IP_VERSION_6:
    enqueue_in6(netdev, pkt);
    break;
  default: {
    rofcore::logging::debug
        << "[rofgtp][cgtprelay][enqueue] unknown packet received, ignoring"
        << std::endl;
  };
  }
...
void cgtprelay::enqueue_in4(rofcore::cnetdev *netdev, rofl::cpacket *pkt) {

  if (sizeof(struct rofl::fipv4frame::ipv4_hdr_t) > pkt->length()) {
    rofcore::logging::debug
        << "[rofgtp][cgtprelay][enqueue_in4] invalid packet received, dropping"
        << std::endl;
    return;
  }
  ...
  rofcore::logging::debug
      << "[rofgtp][cgtprelay][enqueue_in4] IPv4 packet received" << std::endl
      << ipv4;
  ...
  try {
    ...
    cterm_in4 &term =
        cgtpcore::set_gtp_core(dptid).set_term_in4(netdev->get_devname());
    rofcore::logging::debug
        << "[rofgtp][cgtprelay][enqueue_in4] cterm_in4 (src->dst) found"
        << std::endl
        << term;
    ...
    rofcore::logging::debug << "[rofgtp][cgtprelay][enqueue_in4] tagged frame"
                            << std::endl
                            << *pkt;
    ...
    rofl::csockaddr to(
        term.get_label_ingress().get_daddr().get_addr(),
    ...
    rofcore::logging::debug
        << "[rofgtp][cgtprelay][enqueue_in4] socket not found" << std::endl;
  } catch (eGtpCoreNotFound &e) {
    rofcore::logging::debug
        << "[rofgtp][cgtprelay][enqueue_in4] gtpcore not found" << std::endl;
  }

  // release_pkt() is called in method ::enqueue()
}

void cgtprelay::enqueue_in6(rofcore::cnetdev *netdev, rofl::cpacket *pkt) {
  if (sizeof(struct rofl::fipv6frame::ipv6_hdr_t) > pkt->length()) {
    rofcore::logging::debug
        << "[rofgtp][cgtprelay][enqueue_in6] invalid packet received, dropping"
        << std::endl;
    return;
  }
  ...
  rofcore::logging::debug
      << "[rofgtp][cgtprelay][enqueue_in6] IPv6 packet received" << std::endl
      << ipv6;
  ...
  try {
    ...
    cterm_in6 &term =
        cgtpcore::set_gtp_core(dptid).set_term_in6(netdev->get_devname());
    rofcore::logging::debug
        << "[rofgtp][cgtprelay][enqueue_in6] cterm_in6 (src->dst) found"
        << std::endl
        << term;
    ...
    rofcore::logging::debug << "[rofgtp][cgtprelay][enqueue_in6] tagged frame"
                            << std::endl
                            << *pkt;
    ...
    rofcore::logging::debug
        << "[rofgtp][cgtprelay][enqueue_in6] socket not found" << std::endl;
  } catch (eGtpCoreNotFound &e) {
    rofcore::logging::debug
        << "[rofgtp][cgtprelay][enqueue_in6] gtpcore not found" << std::endl;
  }

  // release_pkt() is called in method ::enqueue()
}

void cgtprelay::addr_in4_created(unsigned int ifindex, uint16_t adindex) {
  try {
    ...
    add_socket_in4(roflibs::gtp::caddress_gtp_in4(
        addr.get_local_addr(),
    ...
    rofcore::logging::debug << "[cbasebox][addr_in4_created] link not found"
                            << std::endl;
  } catch (rofcore::crtaddr::eRtAddrNotFound &e) {
    rofcore::logging::debug << "[cbasebox][addr_in4_created] address not found"
                            << std::endl;
  } catch (rofl::eSysCall &e) {
    // ...
  }
...
void cgtprelay::addr_in4_deleted(unsigned int ifindex, uint16_t adindex) {
  try {
    const rofcore::crtaddr_in4 &addr = rofcore::cnetlink::get_instance()
                                           .get_links()
                                           .get_link(ifindex)
    ...
    drop_socket_in4(roflibs::gtp::caddress_gtp_in4(
        addr.get_local_addr(),
        roflibs::gtp::cport(roflibs::gtp::cgtpcore::DEFAULT_GTPU_PORT)));
    ...
    rofcore::logging::debug << "[cbasebox][addr_in4_deleted] link not found"
                            << std::endl;
  } catch (rofcore::crtaddr::eRtAddrNotFound &e) {
    rofcore::logging::debug << "[cbasebox][addr_in4_deleted] address not found"
                            << std::endl;
  } catch (rofl::eSysCall &e) {
    // ...
  }
...
void cgtprelay::addr_in6_created(unsigned int ifindex, uint16_t adindex) {
  try {
    ...
    add_socket_in6(roflibs::gtp::caddress_gtp_in6(
        addr.get_local_addr(),
        roflibs::gtp::cport(roflibs::gtp::cgtpcore::DEFAULT_GTPU_PORT)));
    ...
    rofcore::logging::debug << "[cbasebox][addr_in6_created] link not found"
                            << std::endl;
  } catch (rofcore::crtaddr::eRtAddrNotFound &e) {
    rofcore::logging::debug << "[cbasebox][addr_in6_created] address not found"
                            << std::endl;
  } catch (rofl::eSysCall &e) {
    // ...
  }
...
void cgtprelay::addr_in6_deleted(unsigned int ifindex, uint16_t adindex) {
  try {
    const rofcore::crtaddr_in6 &addr = rofcore::cnetlink::get_instance()
                                           .get_links()
                                           .get_link(ifindex)
    ...
    drop_socket_in6(roflibs::gtp::caddress_gtp_in6(
        addr.get_local_addr(),
        roflibs::gtp::cport(roflibs::gtp::cgtpcore::DEFAULT_GTPU_PORT)));
    ...
    rofcore::logging::debug << "[cbasebox][addr_in6_deleted] link not found"
                            << std::endl;
  } catch (rofcore::crtaddr::eRtAddrNotFound &e) {
    rofcore::logging::debug << "[cbasebox][addr_in6_deleted] address not found"
                            << std::endl;
  } catch (rofl::eSysCall &e) {
    // ...
  }
...
void cgtprelay::handle_timeout(int opaque, void *data) {
  switch (opaque) {
...
void cgtprelay::do_keep_alive() {
  for (std::set<unsigned int>::const_iterator it =
           cgtpcore::get_gtp_core(dptid).get_term_in4_ids().begin();
       it != cgtpcore::get_gtp_core(dptid).get_term_in4_ids().end(); ++it)
    try {
      ...
      for (std::map<rofl::caddress_in4, uint64_t>::const_iterator it =
               term.get_peers_in4().begin();
           it != term.get_peers_in4().end(); ++it) {
        const rofl::caddress_in4 &peer = it->first;

        // hack => send an empty frame also to our peer entity, just to keep the
        // ARP entry alive
        // btw., OpenFlow is a crappy framework ...

        rofl::cmemory *mem = new rofl::cmemory(32);

        set_socket_in4(term.get_label_ingress().get_saddr())
            .send(mem, rofl::csockaddr(peer, 2152));
      }
  ...
  for (std::set<unsigned int>::const_iterator it =
           cgtpcore::get_gtp_core(dptid).get_term_in6_ids().begin();
       it != cgtpcore::get_gtp_core(dptid).get_term_in6_ids().end(); ++it)
    try {
      ...
      for (std::map<rofl::caddress_in6, uint64_t>::const_iterator it =
               term.get_peers_in6().begin();
           it != term.get_peers_in6().end(); ++it) {
        ...
        set_socket_in6(term.get_label_ingress().get_saddr())
            .send(mem, rofl::csockaddr(peer, 2152));
      }
