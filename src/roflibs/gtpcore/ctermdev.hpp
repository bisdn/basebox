...
public:
   ...
   */
  virtual ~ctermdev() {
    for (std::map<rofcore::cprefix_in4, uint64_t>::const_iterator it =
             prefixes_in4.begin();
         it != prefixes_in4.end(); ++it) {
      roflibs::common::openflow::ccookie_owner::release_cookie(it->second);
    }
    for (std::map<rofcore::cprefix_in6, uint64_t>::const_iterator it =
             prefixes_in6.begin();
         it != prefixes_in6.end(); ++it) {
      roflibs::common::openflow::ccookie_owner::release_cookie(it->second);
    }
  };
...
public:
   ...
   *
   */
  void add_prefix_in4(const rofcore::cprefix_in4 &prefix) {
    try {
      if (prefixes_in4.find(prefix) != prefixes_in4.end()) {
        return;
      }
      rofcore::cnetlink::get_instance().add_addr_in4(
          get_ifindex(), prefix.get_addr(), prefix.get_prefixlen());
      prefixes_in4[prefix] =
      ...
      rofcore::logging::debug << "[rofcore][ctermdev][add_prefix_in4] failed "
                                 "to set address via netlink"
                              << std::endl;
    } catch (rofl::eRofDptNotFound &e) {
      rofcore::logging::debug
          << "[rofcore][ctermdev][add_prefix_in4] dpt not found" << std::endl;
    }
  };
   ...
   *
   */
  void drop_prefix_in4(const rofcore::cprefix_in4 &prefix) {
    try {
      if (prefixes_in4.find(prefix) == prefixes_in4.end()) {
        return;
      }
      rofcore::cnetlink::get_instance().drop_addr_in4(
          get_ifindex(), prefix.get_addr(), prefix.get_prefixlen());
      if (STATE_ATTACHED == state) {
        handle_dpt_close(prefix, prefixes_in4[prefix]);
      }
      roflibs::common::openflow::ccookie_owner::release_cookie(
          prefixes_in4[prefix]);
      prefixes_in4.erase(prefix);
    } catch (rofcore::eNetLinkFailed &e) {
      rofcore::logging::debug << "[rofcore][ctermdev][drop_prefix_in4] failed "
                                 "to set address via netlink"
                              << std::endl;
    } catch (rofl::eRofDptNotFound &e) {
      rofcore::logging::debug
          << "[rofcore][ctermdev][drop_prefix_in4] dpt not found" << std::endl;
    }
  };
   ...
   *
   */
  bool has_prefix_in4(const rofcore::cprefix_in4 &prefix) const {
...
public:
   ...
   *
   */
  void add_prefix_in6(const rofcore::cprefix_in6 &prefix) {
    try {
      if (prefixes_in6.find(prefix) != prefixes_in6.end()) {
        return;
      }
      rofcore::cnetlink::get_instance().add_addr_in6(
          get_ifindex(), prefix.get_addr(), prefix.get_prefixlen());
      prefixes_in6[prefix] =
          roflibs::common::openflow::ccookie_owner::acquire_cookie();
      if (STATE_ATTACHED == state) {
      ...
      rofcore::logging::debug << "[rofcore][ctermdev][add_prefix_in6] failed "
                                 "to set address via netlink"
                              << std::endl;
    } catch (rofl::eRofDptNotFound &e) {
      rofcore::logging::debug
          << "[rofcore][ctermdev][add_prefix_in6] dpt not found" << std::endl;
    }
  };
   ...
   *
   */
  void drop_prefix_in6(const rofcore::cprefix_in6 &prefix) {
    try {
      if (prefixes_in6.find(prefix) == prefixes_in6.end()) {
        return;
      }
      rofcore::cnetlink::get_instance().drop_addr_in6(
          get_ifindex(), prefix.get_addr(), prefix.get_prefixlen());
      if (STATE_ATTACHED == state) {
      ...
      rofcore::logging::debug << "[rofcore][ctermdev][drop_prefix_in6] failed "
                                 "to set address via netlink"
                              << std::endl;
    } catch (rofl::eRofDptNotFound &e) {
      rofcore::logging::debug
          << "[rofcore][ctermdev][drop_prefix_in6] dpt not found" << std::endl;
    }
  };
   ...
   *
   */
  bool has_prefix_in6(const rofcore::cprefix_in6 &prefix) const {
...
private:
   ...
   *
   */
  void handle_dpt_open(const rofcore::cprefix_in4 &prefix, uint64_t cookie);

  /**
   *
   */
  void handle_dpt_close(const rofcore::cprefix_in4 &prefix, uint64_t cookie);

  /**
   *
   */
  void handle_dpt_open(const rofcore::cprefix_in6 &prefix, uint64_t cookie);

  /**
   *
   */
  void handle_dpt_close(const rofcore::cprefix_in6 &prefix, uint64_t cookie);

public:
  friend std::ostream &operator<<(std::ostream &os, const ctermdev &termdev) {
    os << rofcore::indent(0) << "<ctermdev "
       << "devname: " << termdev.get_devname() << " "
       << "#in4-prefix(es): " << (int)termdev.prefixes_in4.size() << " "
       << "#in6-prefix(es): " << (int)termdev.prefixes_in6.size() << " >"
       << std::endl;
    rofcore::indent i(2);
    for (std::map<rofcore::cprefix_in4, uint64_t>::const_iterator it =
             termdev.prefixes_in4.begin();
         it != termdev.prefixes_in4.end(); ++it) {
      os << it->first;
    }
    for (std::map<rofcore::cprefix_in6, uint64_t>::const_iterator it =
             termdev.prefixes_in6.begin();
         it != termdev.prefixes_in6.end(); ++it) {
      os << it->first;
    }
    return os;
  };
