...
void cgtpcore::add_gtp_relays() {
  try {
    ...
    for (std::set<unsigned int>::const_iterator it =
             db.get_gtp_relay_ids(dpid).begin();
         it != db.get_gtp_relay_ids(dpid).end(); ++it) {
  ...
  try {
    ...
    for (std::set<unsigned int>::const_iterator it =
      ...
      case 4: {
        ...
        add_term_in4(entry.get_term_id(),
                     entry.get_inject_filter().get_devname(), egress_label,
                     ingress_label, match);
      ...
      case 6: {
        ...
        add_term_in6(entry.get_term_id(),
                     entry.get_inject_filter().get_devname(), egress_label,
                     ingress_label, match);
...
void cgtpcore::handle_dpt_open() {
  try {
    ...
    // install GTP UDP dst rule in GTP local-stage-table (ActionOutput
    // Controller) IPv4
    ...
    // relays and termination points are handled by the relaying and termination
    ...
    rofcore::logging::error
        << "[rofgtp][cgtpcore][handle_dpt_open] unable to find data path"
        << e.what() << std::endl;
  } catch (rofl::eRofSockTxAgain &e) {
    rofcore::logging::error
        << "[rofgtp][cgtpcore][handle_dpt_open] control channel congested"
        << e.what() << std::endl;
  } catch (rofl::eRofBaseNotConnected &e) {
    rofcore::logging::error
        << "[rofgtp][cgtpcore][handle_dpt_open] control channel is down"
        << e.what() << std::endl;
  } catch (...) {
    rofcore::logging::error
        << "[rofgtp][cgtpcore][handle_dpt_open] unexpected error" << std::endl;
  }
}

void cgtpcore::handle_dpt_close() {
  try {
    ...
    // remove GTP UDP dst rule in GTP local-stage-table (ActionOutput
    // Controller) IPv4
    fm.set_table_id(gtp_table_id);
    fm.set_priority(0x4000);
    fm.set_cookie(cookie_miss_entry_ipv4);
...
void cgtpcore::handle_packet_in(rofl::crofdpt &dpt, const rofl::cauxid &auxid,
                                rofl::openflow::cofmsg_packet_in &msg) {
  rofcore::logging::debug << "[cgtpcore][handle_packet_in] pkt received: "
                          << std::endl
                          << msg;
