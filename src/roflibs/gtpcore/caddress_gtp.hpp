...
#include <roflibs/gtpcore/cport.hpp>
...
public:
   ...
   *
   */
  caddress_gtp(const cport &port) : port(port){};
   ...
   *
   */
  caddress_gtp &operator=(const caddress_gtp &addr) {
    if (this == &addr)
      return *this;
    this->port = addr.port;
    return *this;
   ...
   *
   */
  bool operator<(const caddress_gtp &addr) const {
...
public:
   ...
   *
   */
  const cport &get_port() const { return port; };

public:
  friend std::ostream &operator<<(std::ostream &os, const caddress_gtp &addr) {
...
public:
   ...
   *
   */
  caddress_gtp_in4 &operator=(const caddress_gtp_in4 &addr) {
    if (this == &addr)
      return *this;
   ...
   *
   */
  bool operator<(const caddress_gtp_in4 &addr) const {
...
public:
   ...
   *
   */
  caddress_gtp_in6 &operator=(const caddress_gtp_in6 &addr) {
    if (this == &addr)
      return *this;
    caddress_gtp::operator=(addr);
    this->addr = addr.addr;
   ...
   *
   */
  bool operator<(const caddress_gtp_in6 &addr) const {
    return ((this->addr < addr.addr) || (caddress_gtp::operator<(addr)));
  };
