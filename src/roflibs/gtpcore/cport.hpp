...
public:
   ...
   *
   */
  explicit cport(uint16_t port) : port(port){};

  /**
   *
   */
  explicit cport(struct sockaddr_in *sin, socklen_t salen) {
   ...
   *
   */
  explicit cport(struct sockaddr_in6 *sin6, socklen_t salen) {
   ...
   *
   */
  bool operator<(const cport &port) const { return (this->port < port.port); };

public:
  /**
   *
   */
  uint16_t get_value() const { return port; };
...
public:
  friend std::ostream &operator<<(std::ostream &os, const cport &port) {
