...
class cgtprelay : public rofl::csocket_env,
...
public:
   ...
   *
   */
  static cgtprelay &add_gtp_relay(const rofl::cdptid &dptid,
   ...
   *
   */
  static cgtprelay &set_gtp_relay(const rofl::cdptid &dptid,
                                  uint8_t ofp_table_id) {
    if (cgtprelay::gtprelays.find(dptid) == cgtprelay::gtprelays.end()) {
      cgtprelay::gtprelays[dptid] = new cgtprelay(dptid, ofp_table_id);
    }
   ...
   *
   */
  static const cgtprelay &get_gtp_relay(const rofl::cdptid &dptid) {
    if (cgtprelay::gtprelays.find(dptid) == cgtprelay::gtprelays.end()) {
   ...
   *
   */
  static void drop_gtp_relay(const rofl::cdptid &dptid) {
    if (cgtprelay::gtprelays.find(dptid) == cgtprelay::gtprelays.end()) {
      return;
    }
    delete cgtprelay::gtprelays[dptid];
   ...
   *
   */
  static bool has_gtp_relay(const rofl::cdptid &dptid) {
    return (
        not(cgtprelay::gtprelays.find(dptid) == cgtprelay::gtprelays.end()));
  };
...
public:
   ...
   *
   */
  void add_socket_in4(const caddress_gtp_in4 &bindaddr) {
    if (sockets_in4.find(bindaddr) != sockets_in4.end()) {
      drop_socket_in4(bindaddr);
    }
    sockets_in4[bindaddr] =
        rofl::csocket::csocket_factory(rofl::csocket::SOCKET_TYPE_PLAIN, this);
    rofl::cparams sockparams =
        rofl::csocket::get_default_params(rofl::csocket::SOCKET_TYPE_PLAIN);
    ...
    sockparams.set_param(rofl::csocket::PARAM_KEY_LOCAL_HOSTNAME)
        .set_string(bindaddr.get_addr().str());
    sockparams.set_param(rofl::csocket::PARAM_KEY_LOCAL_PORT)
        .set_string(bindaddr.get_port().str());
   ...
   *
   */
  rofl::csocket &set_socket_in4(const caddress_gtp_in4 &bindaddr) {
    if (sockets_in4.find(bindaddr) == sockets_in4.end()) {
   ...
   *
   */
  const rofl::csocket &get_socket_in4(const caddress_gtp_in4 &bindaddr) {
    if (sockets_in4.find(bindaddr) == sockets_in4.end()) {
   ...
   *
   */
  void drop_socket_in4(const caddress_gtp_in4 &bindaddr) {
    if (sockets_in4.find(bindaddr) == sockets_in4.end()) {
      return;
    }
   ...
   *
   */
  bool has_socket_in4(const caddress_gtp_in4 &bindaddr) {
...
public:
   ...
   *
   */
  void add_socket_in6(const caddress_gtp_in6 &bindaddr) {
    if (sockets_in6.find(bindaddr) != sockets_in6.end()) {
      drop_socket_in6(bindaddr);
    }
    sockets_in6[bindaddr] =
        rofl::csocket::csocket_factory(rofl::csocket::SOCKET_TYPE_PLAIN, this);
    rofl::cparams sockparams =
        rofl::csocket::get_default_params(rofl::csocket::SOCKET_TYPE_PLAIN);
    sockparams.set_param(rofl::csocket::PARAM_KEY_DOMAIN)
        .set_string("inet-any");
    sockparams.set_param(rofl::csocket::PARAM_KEY_TYPE).set_string("dgram");
    sockparams.set_param(rofl::csocket::PARAM_KEY_PROTOCOL).set_string("udp");
    sockparams.set_param(rofl::csocket::PARAM_KEY_LOCAL_HOSTNAME)
   ...
   *
   */
  rofl::csocket &set_socket_in6(const caddress_gtp_in6 &bindaddr) {
    if (sockets_in6.find(bindaddr) == sockets_in6.end()) {
   ...
   *
   */
  const rofl::csocket &get_socket_in6(const caddress_gtp_in6 &bindaddr) {
    if (sockets_in6.find(bindaddr) == sockets_in6.end()) {
   ...
   *
   */
  void drop_socket_in6(const caddress_gtp_in6 &bindaddr) {
    if (sockets_in6.find(bindaddr) == sockets_in6.end()) {
      return;
    }
   ...
   *
   */
  bool has_socket_in6(const caddress_gtp_in6 &bindaddr) {
...
public:
   ...
   *
   */
  ctermdev &add_termdev(const std::string &devname) {
   ...
   *
   */
  ctermdev &set_termdev(const std::string &devname) {
    if (termdevs.find(devname) == termdevs.end()) {
      termdevs[devname] = new ctermdev(this, devname, dptid, ofp_table_id);
      try {
        if (STATE_ATTACHED == state) {
   ...
   *
   */
  const ctermdev &get_termdev(const std::string &devname) const {
    if (termdevs.find(devname) == termdevs.end()) {
   ...
   *
   */
  void drop_termdev(const std::string &devname) {
    if (termdevs.find(devname) == termdevs.end()) {
      return;
    }
    delete termdevs[devname];
   ...
   *
   */
  bool has_termdev(const std::string &devname) {
...
public:
  ...
  void handle_dpt_open() {
    ...
    for (std::map<std::string, ctermdev *>::iterator it = termdevs.begin();
         it != termdevs.end(); ++it) {
...
private:
   ...
   *
   */
  void handle_timeout(int opaque, void *data = (void *)0);
...
private:
   ...
   *
   */
  void enqueue_in4(rofcore::cnetdev *netdev, rofl::cpacket *pkt);

  /**
   *
   */
  void enqueue_in6(rofcore::cnetdev *netdev, rofl::cpacket *pkt);

public:
  friend std::ostream &operator<<(std::ostream &os, const cgtprelay &relay) {
    os << rofcore::indent(0) << "<cgtprelay "
       << " #in4: " << (int)relay.sockets_in4.size() << " "
       << " #in6: " << (int)relay.sockets_in6.size() << " >" << std::endl;
    for (std::map<caddress_gtp_in4, rofl::csocket *>::const_iterator it =
             relay.sockets_in4.begin();
         it != relay.sockets_in4.end(); ++it) {
      rofcore::indent i(2);
      os << rofl::indent(0) << *(it->second);
    }
    for (std::map<caddress_gtp_in6, rofl::csocket *>::const_iterator it =
             relay.sockets_in6.begin();
         it != relay.sockets_in6.end(); ++it) {
      rofcore::indent i(2);
      os << rofl::indent(0) << *(it->second);
    }
    return os;
