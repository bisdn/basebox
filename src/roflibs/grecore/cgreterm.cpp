		...
		rofl::crofdpt& dpt, const rofl::cauxid& auxid, rofl::openflow::cofmsg_flow_removed& msg)
{
	...
	if (msg.get_cookie() == cookie_gre_port_shortcut) {
		...
		if (not flags.test(FLAG_GRE_PORT_REDIRECTED)) {
			rofcore::logging::debug << "[cgreterm][handle_flow_removed] installing redirect rule for ingress" << std::endl;
	...
	if (msg.get_cookie() == cookie_gre_tunnel_shortcut) {
		...
		if (not flags.test(FLAG_GRE_TUNNEL_REDIRECTED)) {
			rofcore::logging::debug << "[cgreterm][handle_flow_removed] installing redirect rule for egress" << std::endl;
