...
/*static*/ std::string cgreterm::script_path_init =
    std::string("/var/lib/basebox/greterm.sh");
/*static*/ std::string cgreterm::script_path_term =
    std::string("/var/lib/basebox/greterm.sh");
...
cgreterm::cgreterm(const rofl::cdptid &dptid, uint8_t eth_ofp_table_id,
    ...
    : state(STATE_DETACHED), dptid(dptid), eth_ofp_table_id(eth_ofp_table_id),
      gre_ofp_table_id(gre_ofp_table_id),
      ip_fwd_ofp_table_id(ip_fwd_ofp_table_id),
      idle_timeout(DEFAULT_IDLE_TIMEOUT), gre_portno(gre_portno),
      gre_key(gre_key), gretap(0),
  ...
  std::stringstream ss;
  ss << std::string("gretap") << gre_portno;
...
void cgreterm::handle_packet_in(rofl::crofdpt &dpt, const rofl::cauxid &auxid,
                                rofl::openflow::cofmsg_packet_in &msg) {
  rofcore::logging::debug << "[cgreterm][handle_packet_in] pkt received"
                          << std::endl;
  ...
  if (msg.get_cookie() == cookie_gre_port_redirect) {
    if (!gretap) {
      rofcore::logging::debug
          << "[cgreterm][handle_packet_in] no gretap device, dropping packet "
          << std::endl
          << msg;
      return;
    }

    rofcore::logging::debug << "[cgreterm][handle_packet_in] ingress pkt "
                               "received coming from GRE port"
                            << std::endl
                            << msg.get_packet();
    ...
    *pkt = msg.get_packet();
    ...
    if (not flags.test(FLAG_GRE_PORT_SHORTCUT)) {
      rofcore::logging::debug
          << "[cgreterm][handle_packet_in] installing shortcut rule for ingress"
          << std::endl;
    ...
    if (not flags.test(FLAG_GRE_TUNNEL_SHORTCUT)) {
      rofcore::logging::debug
          << "[cgreterm][handle_packet_in] installing shortcut rule for egress"
          << std::endl;
      ...
      // ip/gre frames received from peer entity are enqueued to ethcore's tap
      // devices
      if (msg.get_cookie() == cookie_gre_tunnel_redirect) {
    rofcore::logging::debug << "[cgreterm][handle_packet_in] egress pkt "
                               "received heading towards GRE port"
                            << std::endl
                            << msg.get_packet();
...
void cgreterm::handle_flow_removed(rofl::crofdpt &dpt,
                                   const rofl::cauxid &auxid,
                                   rofl::openflow::cofmsg_flow_removed &msg) {
  if (msg.get_cookie() == cookie_gre_port_redirect) {
    flags.reset(FLAG_GRE_PORT_REDIRECTED);
  } else if (msg.get_cookie() == cookie_gre_port_shortcut) {
    flags.reset(FLAG_GRE_PORT_SHORTCUT);
    if (not flags.test(FLAG_GRE_PORT_REDIRECTED)) {
      rofcore::logging::debug << "[cgreterm][handle_flow_removed] installing "
                                 "redirect rule for ingress"
                              << std::endl;
      gre_port_redirect(dpt, true);
    }
  } else if (msg.get_cookie() == cookie_gre_tunnel_redirect) {
    flags.reset(FLAG_GRE_TUNNEL_REDIRECTED);
  } else if (msg.get_cookie() == cookie_gre_tunnel_shortcut) {
    flags.reset(FLAG_GRE_TUNNEL_SHORTCUT);
    if (not flags.test(FLAG_GRE_TUNNEL_REDIRECTED)) {
      rofcore::logging::debug << "[cgreterm][handle_flow_removed] installing "
                                 "redirect rule for egress"
                              << std::endl;
...
void cgreterm::enqueue(rofcore::cnetdev *netdev, rofl::cpacket *pkt) {
  try {
    ...
    rofcore::logging::error << "[cgreterm][enqueue] unable to find tap device"
                            << std::endl;
  }
...
void cgreterm::enqueue(rofcore::cnetdev *netdev,
...
void cgreterm_in4::gre_port_redirect(rofl::crofdpt &dpt, bool enable) {
  try {
    ...
    rofcore::logging::error << "[cgreterm_in4][gre_port_redirect] dpt not found"
                            << std::endl;
  } catch (rofl::eRofBaseNotConnected &e) {
    rofcore::logging::error
        << "[cgreterm_in4][gre_port_redirect] control channel is down"
        << std::endl;
  } catch (rofl::eRofSockTxAgain &e) {
    rofcore::logging::error
        << "[cgreterm_in4][gre_port_redirect] control channel congested"
        << std::endl;
  } catch (rofl::RoflException &e) {
    rofcore::logging::error
        << "[cgreterm_in4][gre_port_redirect] unexpected exception caught: "
        << e.what() << std::endl;
  }
}

void cgreterm_in4::gre_port_shortcut(rofl::crofdpt &dpt, bool enable) {
  try {
    rofl::openflow::cofflowmod fm(dpt.get_version_negotiated());
    ...
    if (enable) {
      ...
      fm.set_instructions()
          ...
          .set_exp_body(
              rofl::openflow::experimental::gre::cofaction_exp_body_push_gre(
      ...
      fm.set_instructions()
          ...
          .set_action_set_field(++index)
          .set_oxm(
              rofl::openflow::experimental::gre::coxmatch_ofx_gre_key(gre_key));
      // push vlan, all IP traffic within the datapath carries one
      fm.set_instructions()
          .set_inst_apply_actions()
          .set_actions()
          .add_action_push_vlan(++index);
      fm.set_instructions()
          .set_inst_apply_actions()
          .set_actions()
          .set_action_push_vlan(index)
          .set_eth_type(rofl::fvlanframe::VLAN_CTAG_ETHER);
      // Goto Next Table: ofp_table_id + 1
      fm.set_instructions().add_inst_goto_table().set_table_id(
          ip_fwd_ofp_table_id);
    }
    ...
    rofcore::logging::error << "[cgreterm_in4][gre_port_shortcut] dpt not found"
                            << std::endl;
  } catch (rofl::eRofBaseNotConnected &e) {
    rofcore::logging::error
        << "[cgreterm_in4][gre_port_shortcut] control channel is down"
        << std::endl;
  } catch (rofl::eRofSockTxAgain &e) {
    rofcore::logging::error
        << "[cgreterm_in4][gre_port_shortcut] control channel congested"
        << std::endl;
  } catch (rofl::RoflException &e) {
    rofcore::logging::error
        << "[cgreterm_in4][gre_port_shortcut] unexpected exception caught: "
        << e.what() << std::endl;
  }
}

void cgreterm_in4::gre_tunnel_redirect(rofl::crofdpt &dpt, bool enable) {
  try {
    rofl::openflow::cofflowmod fm(dpt.get_version_negotiated());
    ...
    fm.set_match().set_matches().add_match(
        rofl::openflow::experimental::gre::coxmatch_ofx_gre_version(
            0 /*<< 15*/)); // GRE version 0
    fm.set_match().set_matches().add_match(
        rofl::openflow::experimental::gre::coxmatch_ofx_gre_prot_type(
            GRE_PROT_TYPE_TRANSPARENT_ETHERNET_BRIDGING)); // 0x6558
    fm.set_match().set_matches().add_match(
        rofl::openflow::experimental::gre::coxmatch_ofx_gre_key(
            gre_key)); // GRE key
    ...
    rofcore::logging::error
        << "[cgreterm_in4][gre_tunnel_redirect] dpt not found" << std::endl;
  } catch (rofl::eRofBaseNotConnected &e) {
    rofcore::logging::error
        << "[cgreterm_in4][gre_tunnel_redirect] control channel is down"
        << std::endl;
  } catch (rofl::eRofSockTxAgain &e) {
    rofcore::logging::error
        << "[cgreterm_in4][gre_tunnel_redirect] control channel congested"
        << std::endl;
  } catch (rofl::RoflException &e) {
    rofcore::logging::error
        << "[cgreterm_in4][gre_tunnel_redirect] unexpected exception caught: "
        << e.what() << std::endl;
  }
}

void cgreterm_in4::gre_tunnel_shortcut(rofl::crofdpt &dpt, bool enable) {
  try {
    rofl::openflow::cofflowmod fm(dpt.get_version_negotiated());
    ...
    if (enable) {
      ...
      fm.set_instructions()
          ...
          .set_action_experimenter(index)
          .set_exp_body(
              rofl::openflow::experimental::gre::cofaction_exp_body_pop_gre(
                  0 /* not used for transparent bridging*/));
    ...
    rofcore::logging::error
        << "[cgreterm_in4][gre_tunnel_shortcut] dpt not found" << std::endl;
  } catch (rofl::eRofBaseNotConnected &e) {
    rofcore::logging::error
        << "[cgreterm_in4][gre_tunnel_shortcut] control channel is down"
        << std::endl;
  } catch (rofl::eRofSockTxAgain &e) {
    rofcore::logging::error
        << "[cgreterm_in4][gre_tunnel_shortcut] control channel congested"
        << std::endl;
  } catch (rofl::RoflException &e) {
    rofcore::logging::error
        << "[cgreterm_in4][gre_tunnel_shortcut] unexpected exception caught: "
        << e.what() << std::endl;
  }
}

void cgreterm_in6::gre_port_redirect(rofl::crofdpt &dpt, bool enable) {
  try {
    rofl::openflow::cofflowmod fm(dpt.get_version_negotiated());
    ...
    rofcore::logging::error << "[cgreterm_in6][gre_port_redirect] dpt not found"
                            << std::endl;
  } catch (rofl::eRofBaseNotConnected &e) {
    rofcore::logging::error
        << "[cgreterm_in6][gre_port_redirect] control channel is down"
        << std::endl;
  } catch (rofl::eRofSockTxAgain &e) {
    rofcore::logging::error
        << "[cgreterm_in6][gre_port_redirect] control channel congested"
        << std::endl;
  } catch (rofl::RoflException &e) {
    rofcore::logging::error
        << "[cgreterm_in6][gre_port_redirect] unexpected exception caught: "
        << e.what() << std::endl;
  }
}

void cgreterm_in6::gre_port_shortcut(rofl::crofdpt &dpt, bool enable) {
  try {
    rofl::openflow::cofflowmod fm(dpt.get_version_negotiated());
    ...
    rofcore::logging::error << "[cgreterm_in6][gre_port_shortcut] dpt not found"
                            << std::endl;
  } catch (rofl::eRofBaseNotConnected &e) {
    rofcore::logging::error
        << "[cgreterm_in6][gre_port_shortcut] control channel is down"
        << std::endl;
  } catch (rofl::eRofSockTxAgain &e) {
    rofcore::logging::error
        << "[cgreterm_in6][gre_port_shortcut] control channel congested"
        << std::endl;
  } catch (rofl::RoflException &e) {
    rofcore::logging::error
        << "[cgreterm_in6][gre_port_shortcut] unexpected exception caught: "
        << e.what() << std::endl;
  }
}

void cgreterm_in6::gre_tunnel_redirect(rofl::crofdpt &dpt, bool enable) {
  try {
    rofl::openflow::cofflowmod fm(dpt.get_version_negotiated());
    ...
    rofcore::logging::error
        << "[cgreterm_in6][gre_tunnel_redirect] dpt not found" << std::endl;
  } catch (rofl::eRofBaseNotConnected &e) {
    rofcore::logging::error
        << "[cgreterm_in6][gre_tunnel_redirect] control channel is down"
        << std::endl;
  } catch (rofl::eRofSockTxAgain &e) {
    rofcore::logging::error
        << "[cgreterm_in6][gre_tunnel_redirect] control channel congested"
        << std::endl;
  } catch (rofl::RoflException &e) {
    rofcore::logging::error
        << "[cgreterm_in6][gre_tunnel_redirect] unexpected exception caught: "
        << e.what() << std::endl;
  }
}

void cgreterm_in6::gre_tunnel_shortcut(rofl::crofdpt &dpt, bool enable) {
  try {
    rofl::openflow::cofflowmod fm(dpt.get_version_negotiated());
    ...
    rofcore::logging::error
        << "[cgreterm_in6][gre_tunnel_shortcut] dpt not found" << std::endl;
  } catch (rofl::eRofBaseNotConnected &e) {
    rofcore::logging::error
        << "[cgreterm_in6][gre_tunnel_shortcut] control channel is down"
        << std::endl;
  } catch (rofl::eRofSockTxAgain &e) {
    rofcore::logging::error
        << "[cgreterm_in6][gre_tunnel_shortcut] control channel congested"
        << std::endl;
  } catch (rofl::RoflException &e) {
    rofcore::logging::error
        << "[cgreterm_in6][gre_tunnel_shortcut] unexpected exception caught: "
        << e.what() << std::endl;
  }
}

void cgreterm_in4::hook_init() {
  try {
    rofl::crofdpt &dpt = rofl::crofdpt::get_dpt(dptid);
    // gre_portname = dpt.get_ports().get_port(gre_portno).get_name();
    std::stringstream sstap;
    sstap << std::string("gretap") << gre_portno;
    gre_tap_portname = sstap.str();
    std::stringstream sstun;
    sstun << std::string("gretun") << gre_portno;
    ...
    rofcore::logging::error
        << "[cgreterm_in4][hook_init] datapath not found, dptid: "
        << dptid.str() << std::endl;
  } catch (rofl::openflow::ePortNotFound &e) {
    rofcore::logging::error
        << "[cgreterm_in4][hook_init] unable to find portno: "
        << (unsigned int)gre_portno << std::endl;
  }
}

void cgreterm_in4::hook_term() {
  try {
    rofl::crofdpt &dpt = rofl::crofdpt::get_dpt(dptid);
    ...
    rofcore::logging::error
        << "[cgreterm_in4][hook_term] datapath not found, dptid: "
        << dptid.str() << std::endl;
  }
}

void cgreterm_in6::hook_init() {
  try {
    rofl::crofdpt &dpt = rofl::crofdpt::get_dpt(dptid);
    // gre_portname = dpt.get_ports().get_port(gre_portno).get_name();
    std::stringstream sstap;
    ...
    rofcore::logging::error
        << "[cgreterm_in6][hook_init] datapath not found, dptid: "
        << dptid.str() << std::endl;
  } catch (rofl::openflow::ePortNotFound &e) {
    rofcore::logging::error
        << "[cgreterm_in6][hook_init] unable to find portno: "
        << (unsigned int)gre_portno << std::endl;
  }
}

void cgreterm_in6::hook_term() {
  try {
    rofl::crofdpt &dpt = rofl::crofdpt::get_dpt(dptid);
    ...
    rofcore::logging::error
        << "[cgreterm_in6][hook_term] datapath not found, dptid: "
        << dptid.str() << std::endl;
  }
}

void cgreterm::execute(std::string const &executable,
                       std::vector<std::string> argv,
                       std::vector<std::string> envp) {
  pid_t pid = 0;
  ...
  if ((pid = fork()) < 0) {
    rofcore::logging::error
        << "[cgreterm][execute] syscall error fork(): " << errno << ":"
